...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	//alert(this.customSettings.isUpload);
	try {
		if (numFilesQueued > 0) {
			
//			swfu.setPostParams({"attCompanyId":$("#attCompanyId").attr("value")});
//			swfu.setPostParams({"attUserId":$("#attUserId").attr("value")});
//			alert("选择好文件后提交");
////			document.getElementById('btnCancel').disabled = "";
//			alert($("#id").attr("value"));
//			this.setPostParams({"mailId":$("#id").attr("value")});
//			alert($("#id").attr("value"));
//			this.startUpload();
		}
	} catch (ex) {
...
function addFileInfo(fileId,message){
	...
	if(row!=null)row.cells[2].innerHTML = "<font color='green'>"+message+"</font>";
}
function addReadyFileInfo(fileid,fileName,message,status){
	$("#infoTable").css("display","block");
	var row = "<tr id="+fileid+" >";
	var col2 = "<td width='350'>"+fileName+"</td>";
	var col4 = "<td align='right' width='150'><a href='#' onclick='deleteFile(\""+fileid+"\");deleteFiles();'>删除</a></td>";
	var row1="</tr>";
	if(status!=null&&status!=""){
		alert(message+","+status);
	}else{
		$("#infoTable").append(row+col2+col4+row1); 
	}
}
