		...
		_fn : { 
			...
			create_node	: function (obj, position, js, callback, is_loaded) {
				...
				$.each(js.data, function (i, m) {
					...
					if(typeof m == "string") { tmp.attr('href','#')[ s.html_titles ? "html" : "text" ](m);tmp.attr("title",m); }// tmp.attr("title",m);为新增加ldx
					else {
						if(!m.attr) { m.attr = {}; }
						if(!m.attr.href) { m.attr.href = '#'; }
						tmp.attr(m.attr)[ s.html_titles ? "html" : "text" ](m.title);tmp.attr("title",m.title); // tmp.attr("title",m.title);为新增加ldx
			...
			load_node_json : function (obj, s_call, e_call) {
				...
				switch(!0) {
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						//alert(s.ajax.url);
						//wj 修改
						//s.ajax.url = "http://192.168.1.99:8080/imatrix/tree.action?treeType=MAN_DEPARTMENT_TREE"
						$.ajax(s.ajax);
						//$.ajax({
						 //      type: "POST",
						//	   url: "http://192.168.1.99:8080/imatrix/tree.action?treeType=MAN_DEPARTMENT_TREE",
						//	   data:{currentId:0},
						//	   dataType : "jsonp",
						//	   jsonpCallback:success_func,
						...
						//		}
						//  });
						//$.getJSON("/imatrix/tree.action?treeType=MAN_DEPARTMENT_TREE&callback=?",function(data){alert(data);});
						break;
				}
			},
			_parse_json : function (js, is_callback) {
				...
				if($.isArray(js)) {
					...
					$.each(js.data, function (i, m) {
						tmp = $("<a>");
						if($.isFunction(m)) { m = m.call(this, js); }
						if(typeof m == "string") { tmp.attr('href','#')[ t ? "html" : "text" ](m);tmp.attr("title",m); }// tmp.attr("title",m);为新增加ldx  
						else {
							if(!m.attr) { m.attr = {}; }
							if(!m.attr.href) { m.attr.href = '#'; }
							tmp.attr(m.attr)[ t ? "html" : "text" ](m.title);tmp.attr("title",m.title);// tmp.attr("title",m.title);为新增加ldx  
							if(m.language) { tmp.addClass(m.language); }
						}
						tmp.prepend("<ins class='jstree-icon'>&#160;</ins>");
	...
	$.jstree.plugin("checkbox", {
		...
		_fn : {
			...
			change_state : function (obj, state) {
				...
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					var _tree_type = $this.parents('div').attr('type');
					//wangjing--修改(能单独选择父节点)
					if(_tree_type=='DEPARTMENT_TREE'||_tree_type=='DEPARTMENT_WORKGROUP_TREE'){
						if(state) {//去除勾选
							if($this.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
								var parentLi = $($this).parents('li');
								for(var i=0;i<parentLi.length;i++){
									var id = $(parentLi[i]).attr("id");
									if(id.indexOf("company")!=-1
											||id.indexOf("allDepartment")!=-1
											||id.indexOf("allWorkGroup")!=-1){
										$(parentLi[i]).removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
									}
								}
								
								var childrenLi = $($this).find('li');
								for(var i=0;i<childrenLi.length;i++){
									var id = $(childrenLi[i]).attr("id");
									if(id.indexOf("company")!=-1
											||id.indexOf("allDepartment")!=-1
											||id.indexOf("allWorkGroup")!=-1){
										$(childrenLi[i]).removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
									}
								}
								
								var currentId = $($this).attr("id");
								if(currentId.indexOf("company")!=-1
										||currentId.indexOf("allDepartment")!=-1
										||currentId.indexOf("allWorkGroup")!=-1){
									$($this).removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
								}
