...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		jQuery_formValidator_initConfig.pop();
		jQuery_formValidator_initConfig.push( settings );
	},
	
	//如果validator对象对应的element对象的validator属性追加要进行的校验。
	appendValid : function(id, setting )
	{//alert(id);alert(setting);
	...
	//标准表单调用添加验证
	validateFormFlowable : function(data,map){
		if(typeof(data)=='undefined' || data=="") return;
		//data = '{"required_fields":[{id:"oneint"},{id:"onttext",type:"TEXT"},{id:"onedouble",type:"AMOUNT"}],"forbidden_fields":["ontnottext"]}';
		var json = eval("(" + data + ")");
		$.each(json, function(key,values){
			addValidate(key, values, jQuery_formValidator_initConfig[0].formid,map);
		});
	},
	validateDefaultForm : function(data){
		if(typeof(data)=='undefined' || data=="") return;
		//data = '{"required_fields":[{id:"oneint"},{id:"onttext",type:"TEXT"},{id:"onedouble",type:"AMOUNT"}],"forbidden_fields":["ontnottext"]}';
		var json = eval("(" + data + ")");
		$.each(json, function(key,values){
			addValidateTodefaultForm(key, values);
		});
	},
	addFormValidate: function(setting){
		//新格式的验证
...
function validateDataControls(contral_id,plugintype){
	var dataAcqs=$("input[plugintype="+plugintype+"]");
	for(var j=0;j<dataAcqs.length;j++){
		if(!$(dataAcqs[j]).attr('disabled')){
			//data_control以逗号隔开
			var dataControls=$(dataAcqs[j]).attr("data_control").split(",");
			for(var n=0;n<dataControls.length;n++){
				if(dataControls[n]!=""){
					if(contral_id==dataControls[n]){
						$(dataAcqs[j]).attr('disabled', 'disabled');
						break;
					}
				}
			}
...
function addValidate(key, values, formId,map){
	if(key=="required_fields"){
		for(var i = 0; i < values.length; i++){
			var contral_id =values[i].id||values[i];
			if($("#"+contral_id).length!=1)continue;
			if(typeof($("#"+contral_id).get(0).settings)=='undefined'){
				if(map!=undefined){
					$("#"+contral_id+'TipMessage').html("<font color='red'>*</font>");
					$("#"+contral_id).formValidator({onshow:"必填",onfocus:"必填"}).inputValidator({min:1,onerror:map.get(contral_id)});
				}else{
					//typeof values[i].type =="undefined"&&
					if(values[i].type=="NUMBER"){
						$("#"+contral_id).formValidator({onshow:"必填",oncorrect:"正确"}).regexValidator({regexp:"intege",datatype:"enum",onerror:"必须为整数"});
					}else if(values[i].type=="AMOUNT"){
						$("#"+contral_id).formValidator({onshow:"必填",oncorrect:"正确"}).regexValidator({regexp:"decmal",datatype:"enum",onerror:"必须为小数"});
					}else{
						$("#"+contral_id).formValidator({onshow:"必填",onfocus:"必填"}).inputValidator({min:1,onerror:"必填"});
					}
				}
			}else{
				if(map!=undefined){
					var settings = {isvalid : false,min : 1,type : "size",onerror:map.get(contral_id),validatetype:"InputValidator",
							empty:{leftempty:true,rightempty:true}};
					if($("#"+contral_id+'TipMessage').length==1){
						$("#"+contral_id+'TipMessage').html("<font color='red'>*</font>");
					}
					$.formValidator.appendValid(contral_id, settings);
				}else{
					var settings = {isvalid : false,min : 1,type : "size",onerror:"必填",validatetype:"InputValidator",
							empty:{leftempty:true,rightempty:true}};
					$.formValidator.appendValid(contral_id, settings);
				}
	...
	}else if(key=="forbidden_fields"){
		for(var i = 0; i < values.length; i++){
			var contral_id =values[i].id||values[i];
			if(contral_id.indexOf("-")<0 && $("#"+contral_id).length!=1)continue;
			if($("#"+contral_id).attr("nodeName")=="SELECT"){
				$("#"+contral_id).attr('disabled', 'disabled');
			}else{
				if(contral_id.indexOf("-")>=0){
					//单选多选按钮
					if(contral_id.substring(0,contral_id.indexOf("-"))=="radio" ||
						contral_id.substring(0,contral_id.indexOf("-"))=="checkbox"){
						var control_name=contral_id.substring(contral_id.indexOf("-")+1,contral_id.length);
						$("input[name="+control_name+"]").attr('disabled', 'disabled');
					}
				}else{
					if(values[i].type=="DATE" || values[i].type=="TIME"){//字段是日期类型时
						$("#"+contral_id).attr("onfocus","");
					}else{//字段不是日期类型时
						$("#"+contral_id).attr('readonly', 'readonly');
						if($("#"+contral_id+"Div").length==1){
							$("#"+contral_id+"Div").hide();
						}
						//部门人员控件按钮
						$("input[resultid="+contral_id+"]").attr('disabled', 'disabled');
						//数据获取查询字段映射控件
						$("input[data_ref_control="+contral_id+"]").attr('disabled', 'disabled');
						if($("#"+contral_id).attr('type')=="text"){
							//数据获取映射控件
							validateDataControls(contral_id,"DATA_ACQUISITION");
							//数据选择映射控件
							validateDataControls(contral_id,"DATA_SELECTION");
						}
					}
				}
			}
...
function getTipDiv(id){
	return "<span id=\""+id+"Tip\"></span>";
}
function addValidateTodefaultForm(key, values){
	var formId = jQuery_formValidator_initConfig[0].formid;
	if(key=="required_fields"){
		for(var i = 0; i < values.length; i++){
			var contral_id =values[i].id||values[i];
			if(contral_id.indexOf("-")<0 && $("#"+contral_id).length!=1)continue;
			if(($("#"+contral_id).attr("type")=="hidden"))continue;
			if(values[i].id.indexOf("-")>=0){
				//单选多选按钮
				if(values[i].id.substring(0,values[i].id.indexOf("-"))=="radio" ){
					var control_name=values[i].id.substring(values[i].id.indexOf("-")+1,values[i].id.length);
					var inputs = $("input:radio[name="+control_name+"]");
					if(inputs.length==0)continue;
					if($("#"+control_name+"Tip").length==0) $(inputs[inputs.length-1]).parent().after(getTipDiv(control_name));
					 $(inputs).formValidator({tipid:control_name+"Tip",onshow:"请选择",onfocus:"请选择",oncorrect:"输入正确"}).inputValidator({min:1,max:1,onerror:"至少选一个"});
				}else if(values[i].id.substring(0,values[i].id.indexOf("-"))=="checkbox"){
					var control_name=values[i].id.substring(values[i].id.indexOf("-")+1,values[i].id.length);
					var inputs = $("input:checkbox[name="+control_name+"]");
					if(inputs.length==0)continue;
					if($("#"+control_name+"Tip").length==0) $(inputs[inputs.length-1]).parent().after(getTipDiv(control_name));
					 $(inputs).formValidator({tipid:control_name+"Tip",onshow:"请选择",onfocus:"请选择",oncorrect:"输入正确"}).inputValidator({min:1,onerror:"至少选一个"});
				}
			}else{
				if($("#"+contral_id+"Tip").length==0)$("#"+contral_id).after(getTipDiv(contral_id));
				if(values[i].type=="NUMBER"){
					$("#"+contral_id).formValidator({onshow:"必填",oncorrect:"正确"}).regexValidator({regexp:"intege",datatype:"enum",onerror:"必须为整数"});
				}else if(values[i].type=="AMOUNT"){
					$("#"+contral_id).formValidator({onshow:"必填",oncorrect:"正确"}).regexValidator({regexp:"decmal",datatype:"enum",onerror:"必须为小数"});
...
function validateHandler(obj){
	...
	if(obj.controlType.toLowerCase()=="radio"  ){
		//单选多选按钮
			...
			var inputs = $("#"+formId+" input:radio[name="+name+"]");
			if(inputs.length==0)return;
			if(obj.request=="true"){
				if($("#"+name+"Tip").length==0) {
					$(inputs[inputs.length-1]).parent().append(getTip(name));
				}
				 $(inputs).formValidator({tipid:name+"Tip",onshow:"请选择",onfocus:"请选择",oncorrect:"输入正确"}).inputValidator({min:1,max:1,onerror:"至少选一个"});
			}
			...
			var name = obj.id;// 当控件类型是radio或checkbox时 id存的是控件的name属性
			var inputs = $("#"+formId+" input:checkbox[name="+name+"]");
			if(inputs.length==0)return;
			if(obj.request=="true"){
				if($("#"+name+"Tip").length==0) $(inputs[inputs.length-1]).parent().append(getTip(name));
				$(inputs).formValidator({tipid:name+"Tip",onshow:"请选择",onfocus:"请选择",oncorrect:"输入正确"}).inputValidator({min:1,onerror:"至少选一个"});
			}
			if(obj.readonly=="true"){
				$(inputs).attr('disabled', 'disabled');
			}	
	}else{
		if(obj.id!="")if($("#"+obj.id+"Tip").length==0) $("#"+obj.id).after(getTip(obj.id));
		if(obj.request=="true"){
			var formatType = obj.formatType;
			if(formatType=='null'){
				$("#"+obj.id).formValidator({onshow:"必填",onfocus:"请输入",oncorrect:"正确"})
				.inputValidator({min:1,empty:{emptyerror:'错误'},onerror:'错误'});
			}else if(formatType=='string'||formatType=='enum'){
				var format = obj.format;
				var formatTip = obj.formatTip=="null"?"":obj.formatTip;
				var tip = "请输入"+formatTip;
				$("#"+obj.id).formValidator({onshow:tip,onfocus:tip,oncorrect:"正确"})
				.inputValidator({min:1,empty:{emptyerror:tip},onerror:tip})
				.regexValidator({regexp:format,datatype:formatType,onerror:tip});
			}else{
				var tip = "请输入";
				$("#"+obj.id).formValidator({onshow:tip,onfocus:tip,oncorrect:"正确"})
				.inputValidator({min:1,empty:{emptyerror:tip},onerror:tip})
