...
//		var compu = $(inputCals[j]).attr("computational");
//		$("input[pluginType=CALCULATE_COMPONENT]").live('click',function(){parseComputational(compu,cal_id);});
//		
//		for(var i=0;i<arr.length;i++){
//			$("#"+arr[i]).change(function(){
//				parseComputational(compu,cal_id);
//			});
//		}
	}
}
    		...
    		for(var j=0;j<optionArr.length;j++)
    		{
    			if(optionArr[j]!="")  
    			{
    				//添加option
  					...
  					if(typeof cur_val!='undefined' && cur_val==optionArr[j]) $('#'+childArray[i]).attr("value",cur_val+"|"+parentValue);
  				}
  			}
  		}
	}
...
function dataSelectionClick(url,title,dataControls){
	if(controlsExistFun(dataControls)){
		$.colorbox({href:url,iframe:true, width:800, height:600,overlayClose:false,title:title});
//		init_tb(url,title);
	}
}
