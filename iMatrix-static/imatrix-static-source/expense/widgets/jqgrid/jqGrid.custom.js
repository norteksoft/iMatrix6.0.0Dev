...
//列表中格式化百分比
function formatPercent(cellvalue, options, rowObject){
	var format=options.colModel.iMatrix_formatter;
	var decimalPlaces=0;
	if(cellvalue.indexOf(".")!=-1){
		decimalPlaces=format.length-3;
	}
	var num=cellvalue-0;//把“字符串”转换成“数字”类型
	num*=100;
	num=num.toFixed(decimalPlaces);
	return num+"%";
}
function unFormatPercent(cellvalue,options,cell){
    var value=cellvalue;
	value=value.substring(0,value.length-1);
	var num=value-0;//把“字符串”转换成“数字”类型
	var ss=num/100;
    return ss;
}
...
//				}}
		...
		if(obj.isMutiply){
			chkStyle="checkbox";
			if(obj.treeType=="DEPARTMENT_TREE"){
				chkboxType="{'Y' : 's', 'N' : 's' }";
			}else{
...
function formGridTreeCallBack(rowid,hiddenField,treeType,isMutiply){
	if(hiddenField!='' && ( treeType=='COMPANY' || treeType=='MAN_DEPARTMENT_GROUP_TREE' || treeType=='MAN_DEPARTMENT_TREE' || treeType=='MAN_GROUP_TREE')){
		 if(isMutiply){
			$("#"+rowid+"_"+hiddenField).attr("value",ztree.getLoginNames());
		 }else{
			$("#"+rowid+"_"+hiddenField).attr("value",ztree.getLoginName());
		 }
	}else{
		if(isMutiply){
			$("#"+rowid+"_"+hiddenField).attr("value",ztree.getIds());
		}else{
			$("#"+rowid+"_"+hiddenField).attr("value",ztree.getId());
		}
	}
}
