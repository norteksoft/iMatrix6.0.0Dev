...
function addUsers(id){
	if($('#userNamesView').html()=="所有人员"){
		...
		$("#addBtn").colorbox({href:webRoot+'/engine/data-dictionary!addUsers.htm?id='+id+'&showPage="transactors"',iframe:true, innerWidth:700, innerHeight:500,overlayClose:false,title:"选择办理人",onClosed:function(){$("#addBtn").removeClass("cboxElement");}});
...
function addProcesses(id,btnId){
	if(typeof(id)=='undefined' || id == "") id="0";
	$("#"+btnId).colorbox({href:webRoot+'/engine/data-dictionary!addProcesses.htm?id='+id,iframe:true, width:700, height:500,overlayClose:false,title:"选择流程及环节",onClosed:function(){$("#"+btnId).removeClass("cboxElement");}});
...
function addProcessTb(){
	$("#processRange").html('');
	$('#processDiv').html('');
	$("#processDiv").append('<table class="leadTable" id="processTb" style="width:80%"><thead><tr><th style="width:150px;">流程</th><th style="width:80px;">操作</th></tr></thead><tbody id="processViewTb"></tbody></table>');
}
