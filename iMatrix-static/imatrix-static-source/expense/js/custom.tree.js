...
var _split_char="~~";//多个值之间以~~分隔，要兼容老版本中‘-’分隔的情况
/**
 * 页面中只有一颗树
 ...
 * 		2 action中树节点id的拼接规则:以属性separate设置的分隔符隔开多个值,如separate的值为"~~"，则其树节点id应为：值1~~值2~~值3~~值4...
 *      3 js调用规则:
 *      	参数说明:1)必填的参数url
 ...
 *      			11)treeType：其值为jstree或ztree,默认为jstree,jstree表示使用jstree实现自定义树的拼接，ztree表示使用ztree实现自定义树的拼接
 *      			12)separate：分隔符，默认为"~~"
 *      
...
function custom_tree(obj){
	_obj=obj;
	_obj.treeType = "jstree";
	obj.treeType = "jstree";
	_custom_tree(obj,"jstree");
}
/**
 * ztree实现方式
 * @param obj
 * @return
 */
function custom_ztree(obj){
	_obj=obj;
	_obj.treeType = "ztree";
	obj.treeType = "ztree";
	_custom_tree(obj,"ztree");
}

/**
 * 
 * @param obj
 * @param treeType 其值为jstree或ztree
 * @return
 */
function _custom_tree(obj,treeType){
	var rootUrl=webRoot;
	_obj=obj;
	...
	if(typeof(obj.separate)=="undefined"){
		obj.separate = "~~";
	}
	_split_char = obj.separate;
	$.colorbox({href:rootUrl+"/portal/custom-tree.action?treeType="+treeType,
		iframe:true, 
	...
	if(typeof(_crn_obj.nodeInfo)!="undefined"){
		...
		if(n==-1){//n==-1说明nodeInfo中没有prname该参数名
			...
			if(_node_id.indexOf(",")<0){//单选树或多选树中只选了一个节点
				var node_id_vals=_node_id.split("-");//兼容老版本中node的id以obj.separate设的值截取的情况
				if(_node_id.indexOf(_split_char)>=0){//将node的id以"~~"截取
					node_id_vals=_node_id.split(_split_char);//将node的id以"-"截取
				}
				...
				for(var i=0;i<node_ids.length;i++){
					var node_id_vals=node_ids[i].split("-");//兼容老版本中node的id以"-"截取的情况
					if(node_ids[i].indexOf(_split_char)>=0){//将node的id以"~~"截取
						node_id_vals=node_ids[i].split(_split_char);//将node的id以"-"截取
					}
					if(node_id_vals[n]!=''&&typeof(node_id_vals[n])!='undefined'){
...
function getSelectValueByIndex(index){
	...
	var valArr=new Array();
	if(_node_id.indexOf(",")<0){//单选树或多选树中只选了一个节点
		var node_id_vals=_node_id.split("-");//兼容老版本中node的id以"-"截取的情况
		if(_node_id.indexOf(_split_char)>=0){//将node的id以"~~"截取
			node_id_vals=_node_id.split(_split_char);//将node的id以"-"截取
		}
		if(node_id_vals.length>n){
