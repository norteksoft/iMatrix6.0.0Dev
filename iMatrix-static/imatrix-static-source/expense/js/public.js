...
//	if(searchContent.length>0){
	if("INSIDE_QUERY"==_gridTagParams._start_query_sign||"CUSTOM_QUERY"==_gridTagParams._start_query_sign){
		//改变查询div的位置
		changePosition(); 
		//隐藏左侧栏时，重设查询框的宽度
...
var iMatrix={
	showSearchDIV:function(obj){
		if("INSIDE_QUERY"!=_gridTagParams._start_query_sign && "CUSTOM_QUERY"!=_gridTagParams._start_query_sign){//列表设置中启用不查询功能
			alert("请在列表设置中启用查询功能！");
			return;
		}
		
		if("CUSTOM_QUERY"==_gridTagParams._start_query_sign){//列表设置中启用自定义查询功能
			if(!customSearch){
				initSearch();
	...
	saveForm:function(formId){
		var cansave = iMatrix.getSubTableDatas(formId);
		if(cansave){
