...
//	}
	//获得form中的input值
	__jqGridSubmitSearchValue(submitForm,url,id);
	fixedSearchCallBack();
}

function __jqGridSubmitSearchValue(submitForm,url,id){
...
function searchSubSubmit(){
	...
	var _sub_gridId=_subGridTagParams._sub_grid_id;
...
//通用ajaxAnywhere提交
function ajaxSubmit(form, url, zoons, ajaxCallback,arg1,arg2,arg3){
	ajaxAnyWhereSubmit(form,url,zoons,ajaxCallback,arg1,arg2,arg3);
}
//通用消息提示
...
function parseFieldJson(){
	...
	__publicParseFieldJson(jsonFields);
}

function __publicParseFieldJson(jsonFields){
...
//		var submitFormIdFromPage = $($("#searchArea").parent('form')).attr("id");
		var submitFormIdFromPage = $($("#searchTotalable_page_id").parent('form')).attr("id");
...
function changePosition(){
	if($("#containerSearchInput").val()!="false"&&$("#containerSearchInput").val()!=undefined){//嵌入式
		...
		var topHeight = 35;
		var leftPos = 0;
		var uiTabId = $(".ui-tabs").attr("id");
		if(uiTabId!=""&&typeof(uiTabId)!='undefined'){//判断是否有页签
			topHeight = 80;
			leftPos = 12;
		}
		...
		if(typeof(targetPositionLeft)=='undefined'||typeof(targetPositionTop)=='undefined'){
			searchInformationDiv.css('left',leftPos);
			searchInformationDiv.css('top',topHeight);
		}else{
			targetPositionLeft = parseInt(targetPositionLeft)+leftPos;
			targetPositionTop = parseInt(targetPositionTop)+topHeight;
			searchInformationDiv.css('left',targetPositionLeft);
			searchInformationDiv.css('top',targetPositionTop);
		}
		$("#search_box").css("width",$('#opt-content').width()+12+leftPos);
...
function customSearchSubmit(url){ 
	fixedSearchSubmit(_gridTagParams._grid_id, url);
}
...
function getFieldsData(name){
	if(_search_params._custom_field=="")return null;
	var obj=_search_params._custom_field;
