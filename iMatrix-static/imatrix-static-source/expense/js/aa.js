                ...
                for (var $$$$i = 0; $$$$i < scripts.length; $$$$i++) {
                    ...
                    if (script != null) {
                        ...
                        if (script.indexOf("document.write") != -1) {
							...
							//script包含"var ___ignore=true;"时,则不执行以下eval()  liudongxia
                          if(script.indexOf('___ignore=true')==-1){
							//eval("var aaInstanceId = '"+this.id+"'; \n"+script);//本句和下面的“添加替换区域的脚本"重复了
						  }
                            
                            //添加替换区域的脚本
                            if (zones.length != 0) {
	                            var zoneNode = zones[0];
	                            var name = zoneNode.getAttribute("name");
	                            var zoneHolder = document.getElementById("aazone." + name);
	                            if (zoneHolder != null && typeof(zoneHolder) != "undefined") {
	                            	var customerScript = document.createElement("script");
                            		appendChild(customerScript,script);
	                            	zoneHolder.appendChild(customerScript);
	                            }
                            }
                        }
                    }
...
AjaxAnywhere.prototype.showLoadingMessage = function() {
    ...
    if (div == null) {
		...
		$(".ui-layout-center").append(div);//该行原来是document.body.appendChild(div);
        div.id = "AA_" + this.id + "_loading_div";

        div.innerHTML = "&nbsp;正在加载...";//该行原来是div.innerHTML = "&nbsp;Loading...";
