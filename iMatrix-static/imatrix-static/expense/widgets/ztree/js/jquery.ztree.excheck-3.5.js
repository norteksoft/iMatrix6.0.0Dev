...
(function($){
	...
	_view = {
		...
		makeChkClass: function(setting, node) {
			...
			fullStyle = "",
			treeType=setting.data.treeType;
			if(treeType==3||treeType==4||treeType==5){
				if (node.chkDisabled === true) {
					...
					fullStyle = node[checkedKey] ? ((node.check_Child_State === 2 || node.check_Child_State === -1) ? c.FULL:c.FULL) : ((node.check_Child_State < 1)? c.FULL:c.FULL);
				}
