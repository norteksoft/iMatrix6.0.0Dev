...
$.formValidator = 
{
	...
	validateFormFlowable : function(data){
		if(typeof(data)=='undefined' || data=="") return;
		var json = eval("(" + data + ")");
		$.each(json, function(key,values){
			addValidate(key, values, jQuery_formValidator_initConfig[0].formid);
		});
	}
...
function addValidate(key, values, formId){
	if(key=="required_fields"){
		for(var i = 0; i < values.length; i++){
			var settings = {isvalid : false,min : 1,type : "size",onerror:"必填",validatetype:"InputValidator",
					empty:{leftempty:false,rightempty:false,leftemptyerror:'必填',rightemptyerror:'必填'}};
			if(typeof($("#"+values[i]).get(0).settings)=='undefined'){
				$("#"+values[i]).formValidator({onshow:"必填",onfocus:"必填"}).inputValidator({min:1,onerror:"必填"});
			}else{
				$.formValidator.appendValid(values[i], settings);
			}
		}
