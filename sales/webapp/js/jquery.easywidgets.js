...
(function($){
  ...
  function PrepareWidgetBehaviour(widget, widgetOnDemand, settings){
    ...
    if(widgetMenu.html() == null){
      ...
      //widget.find(s.selectors.header).append(menuWrap); //update by J Xiao
      widget.find(s.selectors.header).prepend(menuWrap);
      ...
      //AddWidgetEditLink(widget, widgetMenu, s); //update by J Xiao
  ...
  function AddWidgetCollapseLink(widget, widgetMenu, settings){
    ...
    if(widget.hasClass(s.options.collapsable)){
      if(widget.hasClass(s.options.collapse)){
        ...
        content.hide();
      //INFO隐藏圆角边
        var roundCorner = $(widget).children(".xbottom")[0];
        $(roundCorner).css('display', 'none');
      }else{
        link = MenuLink(
          s.i18n.collapseText,
