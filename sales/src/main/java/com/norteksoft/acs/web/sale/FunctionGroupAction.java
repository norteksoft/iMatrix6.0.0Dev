...
public class FunctionGroupAction extends CRUDActionSupport<FunctionGroup> {
	...
	private Page<FunctionGroup> page = new Page<FunctionGroup>(Page.EACH_PAGE_TWENTY,true);
	...
	public String save() throws Exception {
		...
		//去掉字段两边的空格
		String codek=functionGroup.getCode().trim();
		String namek=functionGroup.getName().trim();
		functionGroup.setCode(codek);
		functionGroup.setName(namek);
	...
	public String getFuncGroupsBySystem() {
		if (systemId != null) {
			...
			if(page.getResult().size()>0&&!(page.getResult().get(0) instanceof FunctionGroup)){
				page.setResult(null);
			}
		}
		return SUCCESS;
