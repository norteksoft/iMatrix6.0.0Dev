...
import com.norteksoft.acs.base.utils.SystemContextUtils;
...
public class  FunctionGroupManager{
	...
	public Long getCompanyId() {
		if(companyId == null){
			return SystemContextUtils.getCompanyId();
		}else 
			return companyId;
	}
	 ...
	 */
	@Transactional(readOnly = true)
	public Page<FunctionGroup> getSearchFunctionGroup(Page<FunctionGroup> page,FunctionGroup functionGroup,boolean deleted,Long systemId) {
		 ...
		 if (functionGroup != null) {
			...
			if (functionGroupId!=null&&!"".equals(functionGroupId) && functionGroupName!=null&&!"".equals(functionGroupName)) {
				    ...
				    functionGroupHql.append(" and fg.businessSystem.id=?");
				    return functionGroupDao.find(page, 
				    		                    functionGroupHql.toString(),
						                        false,
						                        "%" + functionGroupId+ "%", "%" + functionGroupName + "%",systemId);
			}
			...
			if (functionGroupName!=null&&!"".equals(functionGroupName)) {
				   ...
				   return functionGroupDao.find(page, 
						                        functionGroupHql.toString(),
						                        false,
						                        "%" + functionGroupName + "%",systemId);
			}
		}
		 if(systemId!=null){
			 functionGroupHql.append(" and fg.businessSystem.id=?");
			 return functionGroupDao.find(page, functionGroupHql.toString(),false,systemId);
		 }else{
