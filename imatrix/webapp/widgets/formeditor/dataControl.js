function add()
	...
	function del(obj)
	{
	 ...
	 function getListControlData(){
		 ...
		 $("#controlId").attr("value","customList_"+dataSrc);
		 $.ajax({
			   type: "POST",
			   url: "form-view!getTabelColumns.htm",
			   data:{tableName:dataSrc,formControlType:"LIST_CONTROL"},
			   success: function(text, textStatus){
				   var len=$("#contentTable").find("tbody").find("tr").length;
				   for(var i=0;i<len;i++){
					   $("#item_"+(i+1)).attr("value","");
					   $("#size_"+(i+1)).attr("value","30");
	 ...
	 function initListControlRow(){
		 $("#contentTable").find("tbody").append($("#mata").find("tbody").html());
		 __listControlAddRow();
	 }
	 
	 function listControlAddRow(obj){
		 $(obj).parent().parent().after($("#mata").find("tbody").html());
		 __listControlAddRow();
	 }
	 
	 function __listControlAddRow(){
		 var len=$("#contentTable").find("tbody").find("tr").length;
