...
WorkflowButtonGroup.btnAssignTransactorName="选择办理人";
WorkflowButtonGroup.btnChoiceTacheName="选择环节";
...
function WorkflowButtonGroup(id){
	...
	this.btnAssign=new Button('btnAssign',this.id,WorkflowButtonGroup.btnAssignName);//指派按钮
}
...
workflowButtonGroup.btnDrawTask.click = function(taskId){
	$.get(imatrixRoot + "/wf/engine/task!receive.htm", { taskId: taskId },
		function(data){
...
workflowButtonGroup.btnAbandonTask.click = function(taskId){
	$.get(imatrixRoot + "/wf/engine/task!abandonReceive.htm", { taskId: taskId },
		function(data){
