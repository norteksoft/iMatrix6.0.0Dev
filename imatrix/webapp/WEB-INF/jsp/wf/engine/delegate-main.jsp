...
<head>
	...
	//给隐藏域附值
	function insertInputValue(loginName,name,id){
		$("#trustee").attr("value",loginName);
		//$("#trusteeName").attr("value",name);
		$("#trusteeId").attr("value",id);
	}
...
function onClosedFun(){
	...
	var trusteeId = jstree.getId();
	$("#trustee").attr("value",loginName);
	$("#trusteeId").attr("value",trusteeId);
}
...
//					isAppend:"false",
//					callBack:function(){insertInputValue(jstree.getLoginName(),jstree.getName(),jstree.getId());}});
			var zTreeSetting={
					leaf: {
						enable: false
					},
					...
					feedback:{
						enable: true,
				                showInput:"trusteeName",
				                hiddenInput:"trusteeId",
				                append:false
					},
					callback: {
						onClose:onCloseCollBack
					}			
					};
				    popZtree(zTreeSetting);
	}
	//回调函数
	function onCloseCollBack(){
		insertInputValue(ztree.getLoginName(),ztree.getName(),ztree.getId());
	}
	...
	function selectTache(){
		...
		if(processId=="0"){
			...
			custom_ztree({url:webRoot+'/engine/delegate-main!getLink.htm',
