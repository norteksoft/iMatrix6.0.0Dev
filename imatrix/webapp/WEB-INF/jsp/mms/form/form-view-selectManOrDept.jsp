			...
			if("${standard}"=="true"){
				if($("#saveDeptControlValue").get(0).selectedIndex==0){
					...
					parent.deptHtml($("#name").attr("value")
							...
							,$("#styleId").attr("value")
							,$("#dbName").attr("value")
							,$("#saveDeptControlType").attr("value")
							,$("#deptShowBranch").attr("value"));
				}
			}else{
				parent.deptHtml($("#name").attr("value")
						,$("#controlType").attr("value")
				...
				<s:hidden id="occasion" name="occasion" value="changeSource"></s:hidden>
				 <table >
				     <tr>
				      ...
				      <tr>
						...
						<s:if test="standard">
					      ...
					      <tr >
				      		<td>保存信息类型：</td>
				      		<td><s:select id="saveDeptControlType" theme="simple" list="#{'loginName':'登录名/部门名称/工作组名称','userId':'id'}" name="formControl.saveDeptControlType" >
								</s:select>
							</td>
				      		<td></td>
					      </tr>
				     ...
				     <tr>
						<td>是否显示分支：</td>
						<td>
							<s:select id="deptShowBranch" theme="simple" list="#{'false':'否','true':'是'}" name="formControl.deptShowBranch" >
							</s:select>
						</td>
						<td></td>	
					</tr>
