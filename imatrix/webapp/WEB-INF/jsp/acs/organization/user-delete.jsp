    	...
    	function deleteUserToDept(deptId){
			...
			if(uIds==''){
				...
				var branchId = $("#brcheId").val();
			     ...
			     $("#deleteUserAddDeptForm").attr("action","${acsCtx}/organization/user!saveDepartmentToUserDel.action?ids="+userIds+"&departmentId="+deptId+"&departmType="+departmType+"&branchId="+branchId);
...
<div class="opt-body">
	<form id="ajax_from" name="ajax_from" action="" method="post">  
        ...
        <input type="hidden" id="brcheId" name="branchId" value="${branchId}"/>
        <input type="hidden" name="departmType" id="departType" value="${departmType }" />
	</form>
	<aa:zone name="acs_content">
		<div class="opt-btn">
			<security:authorize ifAnyGranted="deleteUserAll">
				<button  class='btn' <s:if test="!canEditUser">style="display: none;"</s:if> onclick="deleteUser('${acsCtx}/organization/user!delete.action');"><span><span><s:text name="user.deleteComplate" /></span></span></button>
		    </security:authorize>
		    <security:authorize ifAnyGranted="addUserDeleteToDepartment"> 
		    	<button  class='btn' <s:if test="!canEditUser">style="display: none;"</s:if>onclick="Dtree();"><span><span><s:text name="department.addDepartment" /></span></span></button>
			...
			<form id="deleteUserAddDeptForm" name="deleteUserAddDeptForm" action="" method="post">
			    <input type="hidden" name="departmentIds" id="departmentIds" >
			         ...
			         <s:if test="containBranches">
						<view:jqGrid url="${acsCtx}/organization/user!deleteList.action?workGroupId=${workGroupId }&departmentId=${departmentId}&departmentIds=${oldDid}&oType=${oldType }&departmType=${departmType }&branchId=${branchId }" pageName="page" code="ACS_USER_SUB_COMPANY" gridId="main_table"></view:jqGrid>
			         </s:if><s:else>
						<view:jqGrid url="${acsCtx}/organization/user!deleteList.action?workGroupId=${workGroupId }&departmentId=${departmentId}&departmentIds=${oldDid}&oType=${oldType }&departmType=${departmType }&branchId=${branchId }" pageName="page" code="ACS_USER" gridId="main_table"></view:jqGrid>
			         </s:else>
					</aa:zone>
			</form>
		</div>
