...
function fileDialogBefore(file,swfObject){
	//表单编辑器中的附件上传控件的fileDialogBefore回调方法
	formAttachFileDialogBefore(file,swfObject);
	var upload_target = swfObject.customSettings.upload_target;
	if(typeof(upload_target)!='undefined'&&upload_target=="divFileProgressContainer"){
		var id=$("#id").attr("value");
		var taskId=$("#taskId").attr("value");
		if(id==""||id==null){
			swfObject.customSettings.isUpload=false;
			alert("请先保存,再上传!");
			swfObject.eventQueue=[];
		}else{
			swfObject.customSettings.isUpload=true;
			swfObject.setPostParams({"id":id,"taskId":taskId});
...
function rewriteMethod(file,swfObject){
	//表单编辑器中的附件上传控件的rewriteMethod回调方法
