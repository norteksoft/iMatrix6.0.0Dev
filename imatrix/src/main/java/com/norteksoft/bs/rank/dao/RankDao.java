...
public class RankDao extends HibernateDao<Superior, Long> {
	...
	public List<Superior> getDirectLeaderByDeptId(List<Long> deptIds, Long companyId){
		StringBuilder hql = new StringBuilder("select dd from Superior dd join dd.dataDictionaryRankUser du ");
		hql.append("where du.subordinateType=? and du.companyId=? ");
		if(deptIds.size()>0){
			hql.append(" and (");
		}
		Object[] paras = new Object[deptIds.size()+2];
		paras[0] = SubordinateType.DEPARTMENT;
		paras[1] = companyId;
		for(int i = 0; i < deptIds.size(); i++){
			if(i != 0) hql.append(" or ");
			hql.append("du.targetId=?");
			paras[i + 2] = deptIds.get(i);
		}
		if(deptIds.size()>0){
			hql.append(")");
		}
	...
	/**
	 * 获得userId对应的所有上下级关系记录
	 * @param userId
	 * @param companyId
	 * @return
	 */
	public List<Superior> getDirectLower(Long userId, Long companyId){
		return find("from Superior dd where dd.userId=? and dd.companyId=? ", 
				 userId, companyId);
	}
