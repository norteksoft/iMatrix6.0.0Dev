...
public class RankAction extends CRUDActionSupport<Superior> {
	...
	public String input() throws Exception{
		if(id!=null){
			...
			for(Subordinate ddru:dataDictRankUsers){
				...
				userInfos.add((ddru.getTargetId()==null?"":ddru.getTargetId())+";"+ddru.getName()+";"+(ddru.getLoginName()==null?"":ddru.getLoginName())+";"+(ddru.getSubordinateType()!=null?ddru.getSubordinateType().getIndex():SubordinateType.USER.getIndex()));
			}
	...
	public String showUserTree() throws Exception {
		...
		if(acsApiManager.hasBranch(ContextUtils.getCompanyId())==true){
			isbranch=true;
		}
		return SUCCESS;
	}
	@Action("validatesup")
	public String validatesup() throws Exception {
		if(rankManager.countSup(usercout,id,uid)>0){
			renderText("cz");
		}
		return null;
	}
	...
	public String getUsercout() {
		return usercout;
	}
	public void setUsercout(String usercout) {
