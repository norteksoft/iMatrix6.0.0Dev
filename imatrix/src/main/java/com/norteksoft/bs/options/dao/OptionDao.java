...
public class OptionDao extends HibernateDao<Option, Long> {
	...
	public Option getDefaultOptionByGroup(Long optionGroupId){
		List<Option> options = find("from Option o where o.companyId=? and  o.optionGroup.id=? and o.selected=? order by o.optionIndex", 
				ContextUtils.getCompanyId(), optionGroupId,true);
		if(options.size()>0){
