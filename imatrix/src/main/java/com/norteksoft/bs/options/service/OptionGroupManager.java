...
public class OptionGroupManager {
    ...
    public Option getDefaultOptionByOptionGroupId(Long optionGroupId){
    	return getDefaultOptionByGroup(optionGroupId);
    }
    
    public Option getDefaultOptionByOptionGroupCode(String groupCode){
    	OptionGroup optionGroup = getOptionGroupByCode(groupCode);
    	if(optionGroup==null)return null;
    	return getDefaultOptionByGroup(optionGroup.getId());
    }
    /**
	 * 根据选项组查询默认选中的选项
	 */
	public Option getDefaultOptionByGroup(Long optionGroupId) {
		Option option = optionDao.getDefaultOptionByGroup(optionGroupId);
		if(option==null){
			List<Option> options = optionDao.getOptionsByGroup(optionGroupId);
			if(options.size()>0)option = options.get(0);
		}
		return option;
	}
