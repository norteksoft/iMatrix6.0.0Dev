...
public class HolidayManager {
	 ...
	 * @return
	 */
	public List<Integer> getMonthSetting(Date date ,Long branchId){
		...
		// 所有设置为特殊日期的
		List<Holiday> setting = holidayDao.getHolidaySetting(startDate, endDate,branchId);
		newCal.add(Calendar.MILLISECOND, 1);
		newCal.add(Calendar.MONTH, -1);
	...
	public List<Holiday> getHolidaySetting(Date startDate, Date endDate,Long branchId){
		return holidayDao.getHolidaySetting(startDate, endDate,branchId);
	}
	...
	public Map<String, List<Date>> getHolidaySettingDays(Date startDate, Date endDate,Long branchId){
		...
		if(branchId!=null&&!haveSettingHoliday(branchId)){
			//分支机构下没有节假日设置就让此分支机构继承集团公司的节假日设置
			branchId=null;
		}
		List<Holiday> holidaySetting = holidayDao.getHolidaySetting(startDate, endDate,branchId);
	...
	/**
	 * 分支机构中是否设置了节假日：true表示设置了，false表示没有设置
	 */
	private boolean haveSettingHoliday(Long branchId){
		List<Holiday> holidaySetting = holidayDao.getHolidaySetting(branchId);
		if(holidaySetting!=null && holidaySetting.size()>0){
			return true;
		}else{
			return false;
		}
	...
	public void saveHoliday(Holiday holiday){
		...
		Holiday hol = holidayDao.getHolidayByDate(holiday.getSpecialDate(),holiday.getSubCompanyId());
		if(hol!=null){
			hol.setDateType(holiday.getDateType());
			hol.setHolidaySettingType(holiday.getHolidaySettingType());
			holiday = hol;
		}
