...
public class WorkflowTaskDao extends HibernateDao<WorkflowTask, Long>{
	...
	public Page<WorkflowTask> getDelegateTasks(Long companyId, String loginName,Long userId, Page<WorkflowTask> page, boolean isEnd){
		...
		//result.setOrder(page.getOrder());
		//result.setOrderBy(page.getOrderBy());
		String hql = "from WorkflowTask t where t.companyId=? and ((t.trustor=? and t.trustorId is null) or (t.trustorId is not null and t.trustorId=?))  and t.effective = true and (t.active=? or t.active=? or t.active=?  or t.active=?) and t.paused=?";
		if(isEnd){
			...
			hql = "from WorkflowTask t where t.companyId=? and ((t.trustor=? and t.trustorId is null) or (t.trustorId is not null and t.trustorId=?))  and t.effective = true and (t.active=? or t.active=? or t.active=? or t.active=?) and t.paused=? ";
	...
	public Page<WorkflowTask> getTaskAsTrustee(Long companyId, String loginName,Long userId, Page<WorkflowTask> page, boolean isEnd){
		Page<WorkflowTask> result = new Page<WorkflowTask>(page.getPageSize(), true);
		result.setPageNo(page.getPageNo());
		String hql = "from WorkflowTask t where t.companyId=? and ((t.transactor = ? and t.transactorId is null) or (t.transactorId is not null and t.transactorId=?)) and t.visible = true and t.effective = true and (t.active=? or t.active=? or t.active=?  or t.active=?) and t.paused=? and t.trustor is not null";
		if(isEnd){
			this.findPage(result,hql,companyId, loginName,userId, TaskState.COMPLETED.getIndex(), TaskState.CANCELLED.getIndex(),TaskState.ASSIGNED.getIndex(),TaskState.HAS_DRAW_OTHER.getIndex(),false);			
		}else{
			hql = "from WorkflowTask t where t.companyId=? and  ((t.transactor = ? and t.transactorId is null) or (t.transactorId is not null and t.transactorId=?))  and t.visible = true and t.effective = true and (t.active=? or t.active=? or t.active=? or t.active=?) and t.paused=? and t.trustor is not null";
			this.findPage(result,hql,companyId, loginName,userId, TaskState.WAIT_TRANSACT.getIndex(), TaskState.WAIT_DESIGNATE_TRANSACTOR.getIndex(),TaskState.DRAW_WAIT.getIndex(),TaskState.WAIT_CHOICE_TACHE.getIndex(),false);
		}
		page = new Page<WorkflowTask>();
		page.setResult(result.getResult());
	...
	public void deleteCountersignHandler(String instanceId,String taskName,Collection<String> users){
		...
		Object[] values=new Object[4+(users.size()*2)];
		String hql = "delete WorkflowTask t where t.processInstanceId=?  and t.name=? and   t.paused=?  and t.processingMode!=? ";
		if(users.size()>0){
			hql =hql+" and (";
		}
		values[0]=instanceId;
		values[1]=taskName; 
		values[2]=false;
		values[3]=TaskProcessingMode.TYPE_READ;
		int i=4;
		for(String user:users){
			transactorInfo.append("(t.transactor=? or t.trustor=?) or ");
			values[i]=user;
			int j=i+1;
			values[j]=user;
			i=j+1;
		}
		if(transactorInfo.indexOf("or")>=0)hql +=transactorInfo.toString().substring(0,transactorInfo.lastIndexOf("or"));
		if(users.size()>0){
			hql +=")";
		}
	...
	public void deleteSignHandler(String instanceId,String taskName,Collection<Long> userIds){
		StringBuilder transactorInfo = new StringBuilder();
		Object[] values=new Object[4+(userIds.size()*2)];
		String hql = "delete WorkflowTask t where t.processInstanceId=?  and t.name=? and   t.paused=?  and t.processingMode!=? ";
		if(userIds.size()>0){
			hql =hql+" and (";
		}
		...
		for(Long userid:userIds){
			transactorInfo.append("(t.transactorId=? or t.trustorId=?) or ");
			values[i]=userid;
			int j=i+1;
			values[j]=userid;
			i=j+1;
		...
		if(transactorInfo.indexOf("or")>=0){
			hql +=transactorInfo.toString().substring(0,transactorInfo.lastIndexOf("or"));
		}
		if(userIds.size()>0){
			hql +=")";
		}
	...
	public void deleteTaskByIds(List<Long> ids){
		if(ids.size()>0){
			StringBuilder taskIdInfo = new StringBuilder();
			Object[] values=new Object[ids.size()];
			String hql = "delete WorkflowTask t where ";
			int i=0;
			for(Long taskId:ids){
				taskIdInfo.append("t.id=? or ");
				values[i]=taskId;
				i++;
			}
			if(taskIdInfo.indexOf("or")>=0)hql +=taskIdInfo.toString().substring(0,taskIdInfo.lastIndexOf("or"));
			this.batchExecute(hql, values);
		}
	}
