...
//	private boolean canceled = false;
	...
	public String personalTasks() throws Exception{
		List<Object[]> typeInfos=taskManager.getTypeInfos( companyId,loginName,userId);
		Integer taskNum=taskManager.getAllTaskNumByUser(companyId,loginName,userId);
...
//		renderText(bl.toString()+taskWebserviceImpl.personalTasks(loginName, companyId, rows));
		...
		renderText(taskWebserviceImpl.personalTasks(loginName,userId, companyId, rows,order));
		return null;
	}
	public String taskMsg() throws Exception{
		tasks.setPageSize(5);
		taskManager.getAllTasksByUser(companyId, loginName,userId, tasks);
	...
	public String list() throws Exception {
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				taskManager.getAllTasksByGroupName(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
			...
			if(tasks.getPageSize()>1){
				taskManager.getAllTasksByCustomType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"待办事宜列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
			...
			if(tasks.getPageSize()>1){
				taskManager.getAllTasksByUserType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"待办事宜列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
	...
	public String completedTasks() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				taskManager.getCompletedTasksByGroupName(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已完成任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
			...
			if(tasks.getPageSize()>1){
				taskManager.getCompletedTasksByCustomType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已完成任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
			...
			if(tasks.getPageSize()>1){
				taskManager.getCompletedTasksByUserType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已完成任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
	...
	public String canceledTasks() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				taskManager.getCancelTasksByGroupName(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已取消任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
			...
			if(tasks.getPageSize()>1){
				taskManager.getCancelTasksByCustomType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已取消任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
			...
			if(tasks.getPageSize()>1){
				taskManager.getCanceledTasksByUserType(getCompanyId(), getLoginName(),getUserId(), tasks,typeName);
				ApiFactory.getBussinessLogService().log("待办事宜", 
						"已取消任务列表", 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
	...
	public String assignTo() throws Exception{
		...
		workflowTaskManager.assigns(ids, transactor);
...
//		String url="http://192.168.1.98:8088/imatrix/"+task.getUrl()+task.getId();
	...
	public String overtimeTask() throws Exception {
		...
		List<WorkflowTask> tasks = workflowTaskManager.getNeedReminderTasks(u.getLoginName(),Long.valueOf(userId),Long.valueOf(companyId));
	...
	public String createTask()throws Exception {
		taskManager.createTask("任务"+new Date().getTime(),"测试普通任务"+new Date().getTime(), "普通任务测试", 28l);
		return "task-list";
	}
	...
	public String taskDetailWindow()throws Exception {
		...
		List<Object[]> taskTypes=taskManager.getTypeInfos(Long.parseLong(companyId),loginName,Long.parseLong(userId));
		...
		for(Object[] taskType:taskTypes){
			taskContent=taskWebserviceImpl.detailTasks(loginName,Long.parseLong(userId), Long.parseLong(companyId), rows,order,String.valueOf(taskType[0]));
	...
	private String portalTypeTree(){
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			...
			ZTreeNode root = new ZTreeNode("workflow_name","0","流程名称", "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			portalTypeTree(typeInfos,treeNodes,"workflow_name");
		}else if(TaskType.DEFAULT_TYPE.toString().equalsIgnoreCase(taskType)){
			typeInfos=taskManager.getTypeInfos(TaskCategory.ACTIVE);
			ZTreeNode root = new ZTreeNode("default_type","0","流程类别", "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			portalTypeTree(typeInfos,treeNodes,"default_type");
		}else{
			typeInfos=taskManager.getTypeInfos(TaskCategory.ACTIVE);
			ZTreeNode root = new ZTreeNode("default_type","0","流程类别", "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			portalTypeTree(typeInfos,treeNodes,"default_type");
			typeInfos=workflowTaskManager.getGroupNames(TaskCategory.ACTIVE);
			root = new ZTreeNode("workflow_name","0","流程名称", "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			portalTypeTree(typeInfos,treeNodes,"workflow_name");
		}
	...
	private void portalTypeTree(List<Object[]> typeInfos,List<ZTreeNode> treeNodes,String parentId){
		...
		for(Object[] objs:typeInfos){
			String typeName=(String)objs[0];
			Long countTask=(Long)objs[1];
			root = new ZTreeNode(typeName,parentId,typeName+"("+countTask+")", "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}
	}
