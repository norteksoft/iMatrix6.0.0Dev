...
public class ModulePageAction extends CrudActionSupport<ModulePage> {
	 ...
	 * 递归菜单父子关系，形成tree
	 */
	private void childMenu(List<Menu> menus,List<ZTreeNode> treeNodes){
		java.util.Collections.sort(menus);
		ZTreeNode node=null;
		for(Menu menu :menus){
			java.util.Collections.sort(menus);
			...
			for(Menu m:children){
				node = new ZTreeNode(m.getId().toString(),menu.getId().toString(),m.getName(), "false", "false", "", "", "folder", "");
				treeNodes.add(node);
			}
			childMenu(children,treeNodes);
		}
	}
