...
public class ButtonDao extends HibernateDao<Button, Long> {
	...
	public List<Button> getUnDeletedButtonsByPageId(Long pageId){
		return this.find("from Button btn where btn.companyId=? and btn.modulePage.id=? and btn.deleted=? order by btn.displayOrder",  ContextUtils.getCompanyId(),pageId,false);
	}
	...
	public void updateButtonByModulePage(Long pageId) {
		this.batchExecute("update Button btn set btn.deleted=? where btn.companyId=? and btn.modulePage.id=? or btn.toPage.id=?", true,ContextUtils.getCompanyId(),pageId,pageId);
	}
	public List<Button> getButtonByModulePage(Long pageId) {
		return this.find("from Button btn  where btn.companyId=? and btn.modulePage.id=? or btn.toPage.id=?", ContextUtils.getCompanyId(),pageId,pageId);
	}
	
	public void updateRebackButtonByModulePage(Long pageId) {
		this.batchExecute("update Button btn set btn.deleted=? where btn.companyId=? and btn.modulePage.id=? or btn.toPage.id=?", false,ContextUtils.getCompanyId(),pageId,pageId);
	}
	public List<Button> findRebackButtonByModulePage(Long pageId) {
		return this.find("from Button btn where (btn.companyId=? and btn.modulePage.id=? and btn.toPage.view.deleted=?) or (btn.toPage.id=?)", ContextUtils.getCompanyId(),pageId,false,pageId);
	}
