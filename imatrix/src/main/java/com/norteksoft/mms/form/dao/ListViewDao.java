...
public class ListViewDao extends HibernateDao<ListView, Long> {
	...
	public List<ListView> getListViewsBySystem(Long menuId){
		String hql = " from ListView lv where lv.companyId=? and lv.menuId=? and lv.deleted=? order by lv.code";
		return this.find(hql, ContextUtils.getCompanyId(), menuId,false);
	}
	...
	public void updateListViewDeletedByTableId(Long dataTableId) {
		this.batchExecute("update ListView v set v.deleted=? where v.companyId=? and v.dataTable.id=?", false,ContextUtils.getCompanyId(),dataTableId);
	}
}
