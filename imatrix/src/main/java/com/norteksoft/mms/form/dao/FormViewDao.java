...
public class FormViewDao extends HibernateDao<FormView, Long> {
	...
	public void getFormViewPageByMenu(Page<FormView> page, Long id) {
	   Long companyId= ContextUtils.getCompanyId();
		String hql = " from FormView fv where fv.companyId="+companyId +
				" and fv.menuId=? and fv.deleted=?";
		this.searchPageByHql(page, hql, id,false);
	}
	public void getDeletedFormViewPage(Page<FormView> page) {
		   Long companyId= ContextUtils.getCompanyId();
			String hql = " from FormView fv where fv.companyId="+companyId +
					" and fv.deleted=?";
			this.searchPageByHql(page, hql, true);
		}
	public List<FormView> getUndeletedViewsByCodeOrderByVersion(String code) {
		String hql = " from FormView fv where fv.code=? and fv.companyId=? and fv.deleted=? and fv.formState!=? and fv.version is not null order by fv.version desc";
		return this.find(hql, code, ContextUtils.getCompanyId(),false,DataState.DRAFT);
	}
	...
	public List<FormView> getViewsByCodeOrderByVersionExceptDraft(String code) {
		...
		return this.find(hql, code, ContextUtils.getCompanyId(),DataState.DRAFT);
	}
	...
	public List<FormView> getFormViewsByCompany(Long companyId){
		String hql = " from FormView fv where fv.companyId=? and fv.formState=? and fv.deleted=? order by fv.code";
		return this.find(hql, companyId,DataState.ENABLE,false);
	}
	...
	public List<FormView> getFormViewsByMenu(Long menuId){
		String hql = " from FormView fv where fv.companyId=? and fv.menuId=? and fv.formState=? and fv.deleted=? order by fv.code";
		return this.find(hql, ContextUtils.getCompanyId(), menuId,DataState.ENABLE,false);
	}
	...
	public List<FormView> getEnableOrDraftFormViewsByMenu(Long menuId){
		String hql = " from FormView fv where fv.companyId=? and fv.menuId=? and (fv.formState=? or fv.formState=?) and fv.deleted=? order by fv.code";
		return this.find(hql, ContextUtils.getCompanyId(), menuId,DataState.ENABLE,DataState.DRAFT,false);
	}
