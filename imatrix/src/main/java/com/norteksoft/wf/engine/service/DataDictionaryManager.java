...
public class DataDictionaryManager {
		...
		for(String user : users){
			if(DataDictAllUsers.ALL_USERS.toString().equals(user)){
				...
				if(type==0){//人员时
					...
					ddu.setLoginName(arr[0]);
					if(arr.length==2){
						ddu.setInfoId(Long.parseLong(arr[1]));
					}
				}else{
	 ...
	 * @return
	 */
	public Set<Long> getCandidateIds(List<Long> dictIds){
		if(dictIds == null) throw new RuntimeException("没有给定查询数据字典办理人的查询条件： 数据字典的ID集合. ");
		Set<Long> ids = new HashSet<Long>();
		for(Long id : dictIds){
			getCandidateIds(id, ids);
		}
		return ids;
	}
	
	public Set<Long> getCandidateIds(Long dictId, Set<Long> ids){
		...
		for(DataDictionaryUser user : users){
			getCandidateIds(user, ids);
		}
		return ids;
	}
	
	private Set<Long> getCandidateIds(DataDictionaryUser user, Set<Long> ids){
		switch (user.getType()){
			//case PERSON : ;
			case USER : ids.add(user.getInfoId()); break;
			//case DEPARTMENT : ;
			case DEPARTMENT : getCandidateIdsByDept(user.getInfoId(), ids); break;
			//case WORKGROUP : ;
			case WORKGROUP : getCandidateIdsByWorkgroup(user.getInfoId(), ids); break;
			//case RANK : ;
			case RANK : getCandidateIdsByRank(user.getInfoId(), ids); break;	
		}
		return ids;
	}
	
	private void getCandidateIdsByRank(Long rankId, Set<Long> ids){
	...
	private void getCandidateIdsByWorkgroup(Long wgId, Set<Long> ids){
	...
	private void getCandidateIdsByDept(Long deptId, Set<Long> ids){
	...
	public List<Long> getCandidateIds(String title){
		if(StringUtils.isEmpty(title)) throw new RuntimeException("没有给定查询数据字典办理人登录名的查询条件： 数据字典标题. ");
		//去掉重复人员
		Set<Long> sr=new HashSet<Long>();
		List<DataDictionaryUser> ddus=dataDictionaryDao.getCandidate(title);
		for(DataDictionaryUser ddu:ddus){
			if(ddu.getType().equals(DataDictUserType.USER)){//人员时
				sr.add(ddu.getInfoId());
			}else if(ddu.getType().equals(DataDictUserType.DEPARTMENT)){
				List<User> list=ApiFactory.getAcsService().getUsersByDepartmentId(ddu.getInfoId());
				for(User user:list){
					sr.add(user.getId());
		...
		users.addAll(sr);
		//得到有序的登录名集合
		List<Long> resultUsers=new ArrayList<Long>();
		if(users.size()>0){
			List<User> results=new ArrayList<User>();
			for(Long uId:users){
				results.add(ApiFactory.getAcsService().getUserById(uId));
			}
			Collections.sort(results, new Comparator<User>() {
				public int compare(User user1, User user2) {
					if(user1.getWeight()<user2.getWeight()){
	...
	public List<String> getCandidateIdsAddition(List<Long> dictIds) {
		...
		for(Long dictId:dictIds){
			getCandidateIdsAddition(dictId,ids);
		}
		return ids;
	}
	public List<String> getCandidateIdsAddition(Long dictId, List<String> loginNames){
		...
		List<DataDictionaryUser> users = dataDictionaryUserDao.find("from DataDictionaryUser u where u.companyId=? and u.dictId=?", getCompanyId(), dictId);
		for(DataDictionaryUser user : users){
			getCandidateIdsAddition(user, loginNames,dd.getRemark());
		}
		return loginNames;
	}
	
	private List<String> getCandidateIdsAddition(DataDictionaryUser user, List<String> loginNames,String remark){
		switch (user.getType()){
		case USER : loginNames.add(user.getInfoId()+":"+remark); break;
		case DEPARTMENT : getCandidateIdsAdditionsByDept(user.getInfoId(), loginNames,remark); break;
		case WORKGROUP : getCandidateIdsAdditionsByWorkgroup(user.getInfoId(), loginNames,remark); break;
			
		}
		return loginNames;
	}
	
	private void getCandidateIdsAdditionsByWorkgroup(Long wgId,List<String> loginNames,String remark){
		List<User> users = ApiFactory.getAcsService().getUsersByWorkgroupId(wgId);
		for(User user : users){
			loginNames.add(user.getId()+":"+remark);
		}
	}
	
	private void getCandidateIdsAdditionsByDept(Long deptId,List<String> loginNames,String remark){
		List<User> users = ApiFactory.getAcsService().getUsersByDepartmentId(deptId);
		for(User user : users){
			loginNames.add(user.getId()+":"+remark);
		}
	...
	public HashMap<Long,String> getUserIds(Long dictId, HashMap<Long,String> ids){
		if(dictId == null) throw new RuntimeException("没有给定查询数据字典办理人登陆名和办理人的查询条件： 数据字典的ID. ");
		if(ids == null) ids = new HashMap<Long, String>();
		List<DataDictionaryUser> users = dataDictionaryUserDao.find("from DataDictionaryUser u where u.companyId=? and u.dictId=?", getCompanyId(), dictId);
		for(DataDictionaryUser user : users){
			getUserIds(user, ids);
		}
	...
	private HashMap<Long,String> getUserIds(DataDictionaryUser user, HashMap<Long,String> ids){
		switch (user.getType()){
			//case PERSON : ;
			case USER : ids.put(user.getInfoId(), user.getInfoName()); break;
			//case DEPARTMENT : ;
			case DEPARTMENT : getUserIdsNamesByDept(user.getInfoId(), ids); break;
			//case WORKGROUP : ;
			case WORKGROUP : getUserIdsByWorkgroup(user.getInfoId(), ids); break;
			//case RANK : ;
			case RANK : getUserIdsByRank(user.getInfoId(), ids); break;	
		}
		return ids;
	}
	
	private void getUserIdsByRank(Long rankId, HashMap<Long,String> ids){
		List<Subordinate> ddrs = rankUserDao.getDataDictRankUsersByRankId(rankId);
		for(Subordinate ddr : ddrs){
			User user=ApiFactory.getAcsService().getUserById(ddr.getTargetId());
	...
	private void getUserIdsByWorkgroup(Long wgId, HashMap<Long,String> ids){
		List<User> users = ApiFactory.getAcsService().getUsersByWorkgroupId(wgId);
		for(User user : users){
			ids.put(user.getId(), user.getName());
		}
	...
	private void getUserIdsNamesByDept(Long deptId, HashMap<Long,String> ids){
		List<User> users = ApiFactory.getAcsService().getUsersByDepartmentId(deptId);
		for(User user : users){
			ids.put(user.getId(), user.getName());
		}
