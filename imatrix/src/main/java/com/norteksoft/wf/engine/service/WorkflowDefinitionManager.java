...
public class WorkflowDefinitionManager {
    ...
    public boolean isSuperWf(){
    	Set<User> wfManager=acsUtils.getUsersByRoleIncludeTrustedRole(ContextUtils.getSystemId("wf"), ContextUtils.getCompanyId(), WF_MANAGER_ROLE_CODE);//工作流管理员列表
		boolean isSuperWf=false;
		for(User user:wfManager){
			if(user.getId().equals(ContextUtils.getUserId())){//如果当前用户既是流程管理员，又是流程定义管理员，则取工作流管理员权限
				return false;
			}
		}
		Set<User> defManagers=acsUtils.getUsersByRoleIncludeTrustedRole(ContextUtils.getSystemId("wf"), ContextUtils.getCompanyId(), DEF_MANAGER_ROLE_CODE);//流程定义管理员
		for(User user:defManagers){
			if(user.getId().equals(ContextUtils.getUserId())){
				isSuperWf=true;
				break;
			}
		}
    ...
    public void getWfDefinitions(Page<WorkflowDefinition> page,String vertionType,String adminCode,Long adminId){
    	boolean isSuperWf=isSuperWf();
    	workflowDefinitionDao.getWfDefinitions(page, getCompanyId(),vertionType,adminCode,isSuperWf,adminId);
    }
    ...
    public void getWfDefinitions(Page<WorkflowDefinition> page, Long type,String vertionType,String adminCode,Long adminId) {
    	boolean isSuperWf=isSuperWf();
    	workflowDefinitionDao.getWfDefinitions(page, getCompanyId(),type,vertionType,adminCode,isSuperWf,adminId);
	}
    ...
    public void getWfDefinitionsBySystemId(Page<WorkflowDefinition> page, Long systemId,String vertionType,String adminCode,Long adminId) {
    	boolean isSuperWf=isSuperWf();
    	workflowDefinitionDao.getWfDefinitionsBySystemId(page, getCompanyId(),systemId,vertionType,adminCode,isSuperWf,adminId);
	}
    ...
    private void setWfDefinitionBaseInfo(WorkflowDefinition wfd, Map<String, String> props){
    	...
    	if(wfd.getId()==null){
			setCreatorInfo(wfd,props.get(DefinitionXmlParse.CURRENTOR_NAME),props.get(DefinitionXmlParse.CURRENTOR_LOGINNAME),props);
    	}
    	wfd.setFormCode(props.get(ProcessProperties.WF_FORM_CODE.toString()));
...
//    	张三/北科汇智[zhangsan~~nortek]
    	String admin = props.get(ProcessProperties.WF_ADMIN.toString());
    	if(admin != null){
    		String[] names = admin.split(DELTA_START_REGEXP);
    		String adminName = names[0];
    		String[] userinfos = names[1].replace(DELTA_END, EMPTY_STRING).split(CommonStrings.LOGINNAME_COMPANY_SPLIT);
    		if(userinfos.length==1){//兼容老版本问题
    			String loginName = userinfos[0];
    			com.norteksoft.product.api.entity.User adminUser = ApiFactory.getAcsService().getUserByLoginName(loginName);
    			if(adminUser!=null){
    				wfd.setAdminName(adminName);
    				wfd.setAdminLoginName(adminUser.getLoginName());
    				wfd.setAdminId(adminUser.getId());
    			}
    		}else{
    			String loginName = userinfos[0];
    			String companyCode = userinfos[1];
    			com.norteksoft.product.api.entity.User adminUser = ApiFactory.getAcsService().getUserByLoginNameAndBranchCode(loginName, companyCode);
    			if(adminUser!=null){
    				wfd.setAdminName(adminName);
    				wfd.setAdminLoginName(adminUser.getLoginName());
    				wfd.setAdminId(adminUser.getId());
    	...
    	String branchCode = props.get(DefinitionXmlParse.BRANCH_CODE);
    	if(StringUtils.isNotEmpty(branchCode)){
    		if(!branchCode.equals(ContextUtils.getCompanyCode())){//不是集团公司才设置分支机构id
    			Department dept = ApiFactory.getAcsService().getDepartmentByCode(branchCode);
    			if(dept!=null){
    				wfd.setSubCompanyId(dept.getId());
    				wfd.setSubCompanyName(dept.getName());
    			}else{
    				wfd.setSubCompanyId(null);
        			wfd.setSubCompanyName(ContextUtils.getCompanyName());
    			}
    		}else{
    			wfd.setSubCompanyId(null);
    			wfd.setSubCompanyName(ContextUtils.getCompanyName());
    ...
    private void setCreatorInfo(WorkflowDefinition wfd,String creatorName,String creator,Map<String, String> props){
    	wfd.setCreatorName(creatorName);
		if(creator.indexOf("[")>=0){
			//creator:loginName[companyCode]
			String[] creatorInfo = creator.split(DELTA_START_REGEXP);
			String companyCode = creatorInfo[1].substring(0,creatorInfo[1].lastIndexOf("]"));
			com.norteksoft.product.api.entity.User u = ApiFactory.getAcsService().getUserByLoginNameAndBranchCode(creatorInfo[0], companyCode);
			wfd.setCreator(creatorInfo[0]);
			if(u!=null){
				wfd.setCreatorId(u.getId());
			}else{
				if(wfd.getId()!=null){
					setCreatorInfo(wfd,props.get(DefinitionXmlParse.CURRENTOR_NAME),props.get(DefinitionXmlParse.CURRENTOR_LOGINNAME),props);
				}
			...
			com.norteksoft.product.api.entity.User u = ApiFactory.getAcsService().getUserByLoginName(creator);
			wfd.setCreator(creator);
			if(u!=null){
				wfd.setCreatorId(u.getId());
			}else{
				if(wfd.getId()!=null){
...
//		}
	...
	public void saveWfBasic(WorkflowDefinition definition,String subCompanyCode){
		if(subCompanyCode!=null){
			if(!subCompanyCode.equals(ContextUtils.getCompanyCode())){//集团公司则分支机构id不应设值
				Department branchDept = ApiFactory.getAcsService().getDepartmentByCode(subCompanyCode);
				if(branchDept!=null){
					definition.setSubCompanyId(branchDept.getId());
					definition.setSubCompanyName(branchDept.getName());
				}
			}else{
				definition.setSubCompanyId(null);
			}
		}else{
			definition.setSubCompanyId(null);
		}
		if(StringUtils.isEmpty(definition.getSubCompanyName()))definition.setSubCompanyName(ContextUtils.getCompanyName());
		...
		try{
			...
			if(defFile!=null){
				...
				if(hasBranch){
					com.norteksoft.product.api.entity.User adminUser = null;
					if(definition.getAdminId()==null){
						adminUser = ApiFactory.getAcsService().getUserByLoginName(definition.getAdminLoginName());
					}else{
						adminUser = ApiFactory.getAcsService().getUserById(definition.getAdminId());
					}
					if(adminUser!=null){
						String companyCode = null;
						if(adminUser.getSubCompanyId()==null){
							companyCode = ContextUtils.getCompanyCode();
						}else{
							Department branch = ApiFactory.getAcsService().getDepartmentById(adminUser.getSubCompanyId());
							if(branch!=null)companyCode = branch.getCode();
						}
						propElement.setText(definition.getAdminName()+"["+definition.getAdminLoginName()+CommonStrings.LOGINNAME_COMPANY_SPLIT+companyCode+"]");
					}
				}else{
				...
				List<Department> branches = ApiFactory.getAcsService().getBranchs();
				if(branches.size()>0){
					propElement = Dom4jUtils.getSubElementByName(basePropElement, "branch-name");
					if(propElement!=null){
						propElement.setText(definition.getSubCompanyName());
					}else{
						Element element = basePropElement.addElement("branch-name");
						element.addText(definition.getSubCompanyName());
					}
					
					propElement = Dom4jUtils.getSubElementByName(basePropElement, "branch-code");
					if(propElement!=null){
						propElement.setText(subCompanyCode);
					}else{
						Element element = basePropElement.addElement("branch-code");
						element.addText(subCompanyCode);
					}
				}
