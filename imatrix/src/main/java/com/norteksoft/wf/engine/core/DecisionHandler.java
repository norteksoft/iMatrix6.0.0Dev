...
public class DecisionHandler implements org.jbpm.api.jpdl.DecisionHandler {
	...
	private String previousTransactorId;
	private String currentTransactor;
	private Long currentTransactorId;
	...
	public  Boolean parseExpression(String express){
		...
		upc.setCurrentTransactorId(currentTransactorId);
		upc.setPreviousTransactor(previousTransactor);
		if(StringUtils.isNotEmpty(previousTransactorId))upc.setPreviousTransactorId(Long.parseLong(previousTransactorId));
	...
	private void init(OpenExecution execution){
		...
		currentTransactorId = ContextUtils.getUserId();
