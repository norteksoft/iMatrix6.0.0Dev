...
public class GeneralTransitionListener implements EventListener{
	...
	private String previousTransactorId;//上一环节办理人 id 
	...
	public void notify(EventListenerExecution execution) throws Exception {
		...
		processAdminId = workflowDefinition.getAdminId();
	...
	private Set<String> getEmailsInformCondition(WorkflowInstance wi){
		...
		UserParseCalculator upc = WebUtil.getUserParseInfor(wi.getProcessInstanceId(),creator,creatorId,processAdmin,processAdminId);
	...
	private Object saveAutomaticallyFilledFieldEntity(WorkflowInstance wi,Object entity) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		//当是java.sql.Date时，自动填写时有问题，所以改为该写法，不用BeanUtils.setProperty方法
	...
	private String getValue(AutomaticallyFilledField aff){
		...
		for(int i=0;i<strs.length;i++){
			...
			if(CommonStrings.PREVIOUS_TRANSACTOR.equals(strs[i])){
				if(StringUtils.isEmpty(previousTransactorId)){
					builder.append(previousTransactor);
				}else{
					builder.append(previousTransactorId);
				}
			}else if(CommonStrings.PREVIOUS_TRANSACTOR_NAME.equals(strs[i])){
				com.norteksoft.product.api.entity.User user = null;
				if(StringUtils.isEmpty(previousTransactorId)){
					user = ApiFactory.getAcsService().getUserByLoginName(previousTransactor);
				}else{
					user = ApiFactory.getAcsService().getUserById(Long.parseLong(previousTransactorId));
				}
	 ...
	 * @return
	 */
	private Long getLongValue(AutomaticallyFilledField aff){
		String condition = aff.getValue();
		if(CommonStrings.PREVIOUS_TRANSACTOR_ID.equals(condition)){
			if(StringUtils.isEmpty(previousTransactorId)){
				User user = ApiFactory.getAcsService().getUserByLoginName(previousTransactor);
				return user.getId();
			}else{
				return Long.parseLong(previousTransactorId);
			}
		}else{
	...
	private void getVariables(OpenExecution execution){
		...
		log.info("开始获取变量");
		creator = execution.getVariable(CommonStrings.CREATOR).toString();
		if(creator!=null){
		...
		creatorId = execution.getVariable(CommonStrings.CREATOR_ID)==null?null:execution.getVariable(CommonStrings.CREATOR_ID).toString();
		if(creatorId!=null&&creatorId.contains(CommonStrings.SUB_PROCESS_CREATOR_TRUSTOR)){//表示是委托任务(trustorId-:-trusteeId)，只有将子流程环节委托出去才会走该判断
		...
		if(obj==null){//上一环节办理人委托人为空，取办理人
			...
			if(obj!=null){
				previousTransactorId = obj.toString();
			}
		}else{//上一环节办理人委托人即真实办理人
			previousTransactorId = obj.toString();
		}
		log.info("从变量中获取上一环节办理人previousTransactor为：" + previousTransactor);
		log.info("从变量中获取上一环节办理人previousTransactorId为：" + previousTransactorId);
	}
}
