...
public class DelegateMainManager {
	...
	public Page<TrustRecord> getPageDelegateMain(Page<TrustRecord> page){
		...
		return delegateMainDao.findPage(page, "from TrustRecord d where d.companyId=? and ((d.trustor=? and d.trustorId is null) or (d.trustorId is not null and d.trustorId =?))", getCompanyId(),getLonginName(),ContextUtils.getUserId());
	}
	...
	public void getReceiveDelegate(Page<TrustRecord> page){
		delegateMainDao.findPage(page, "from TrustRecord d where d.companyId=? and ((d.trustee=? and d.trusteeId is null) or (d.trusteeId is not null and d.trusteeId=?)) and d.state=?",
				getCompanyId(),getLonginName(),ContextUtils.getUserId(),TrustRecordState.EFFICIENT);
	}
	...
	/**
	 * 通过委托人的id，流程 定义ID，环节名
	 * @param myLoadName
	 * @param flowId
	 * @param linkName
	 */
	public User getDelegateUser(Long companyId,String myLoadName,Long myLoadId,String flowId,String linkName){
		...
		String loanName = getDelegateMainName(companyId,myLoadName,myLoadId,cal.getTime());
		if(StringUtils.isNotEmpty(loanName)){
			if(loanName.matches("^-?\\d+$")){//表示是用户id
				return ApiFactory.getAcsService().getUserById(Long.parseLong(loanName));
			}else{
				return ApiFactory.getAcsService().getUserByLoginName(loanName);
			}
		}
		List<TrustRecord> trustRecords=new ArrayList<TrustRecord>();
		String hql = null;
		if(myLoadId==null&&myLoadName!=null){//兼容历史数据
			hql = "from TrustRecord d where d.companyId=?  and d.state=? and d.trustor=? and d.processId=? and (d.activityName=? or d.activityName like ? or d.activityName like ? or d.activityName like ?) and d.beginTime<=? and d.endTime>=? and d.style=1";
			trustRecords=this.delegateMainDao.find(hql,
					companyId,TrustRecordState.EFFICIENT,myLoadName,flowId,linkName,linkName+",%","%,"+linkName+",%","%,"+linkName,cal.getTime(),cal.getTime());
		}else if(myLoadId!=null&&myLoadName!=null){//考虑兼容历史数据
			hql = "from TrustRecord d where d.companyId=?  and d.state=? and ((d.trustor=? and d.trustorId is null) or d.trustorId=?) and d.processId=? and (d.activityName=? or d.activityName like ? or d.activityName like ? or d.activityName like ?) and d.beginTime<=? and d.endTime>=? and d.style=1";
			trustRecords=this.delegateMainDao.find(hql,
					companyId,TrustRecordState.EFFICIENT,myLoadName,myLoadId,flowId,linkName,linkName+",%","%,"+linkName+",%","%,"+linkName,cal.getTime(),cal.getTime());
		}else if(myLoadName==null&&myLoadId!=null){//生成任务时，myLoadName直接传null过来，便于处理
			hql = "from TrustRecord d where d.companyId=?  and d.state=? and d.trustorId=? and d.processId=? and (d.activityName=? or d.activityName like ? or d.activityName like ? or d.activityName like ?) and d.beginTime<=? and d.endTime>=? and d.style=1";
			trustRecords=this.delegateMainDao.find(hql,
					companyId,TrustRecordState.EFFICIENT,myLoadId,flowId,linkName,linkName+",%","%,"+linkName+",%","%,"+linkName,cal.getTime(),cal.getTime());
		}
		if(trustRecords.size()>0){
			if(trustRecords.get(0).getTrusteeId()==null){
				return ApiFactory.getAcsService().getUserByLoginName(trustRecords.get(0).getTrustee());
			}else{
				return ApiFactory.getAcsService().getUserById(trustRecords.get(0).getTrusteeId());
			}
		}
		return null;
	}
	 ...
	 * 全权委托
	 */
	public String getDelegateMainName(Long companyId,String myLoadName,Long myLoadId, Date time){
		String hql = null;
		TrustRecord main=null;
		if(myLoadId==null&&myLoadName!=null){//兼容历史数据
			hql = "from TrustRecord d where d.companyId=? and d.state=? and d.trustor=?  and d.beginTime<=? and d.endTime>=? and d.style=2";
			main=this.delegateMainDao.findUnique(hql, companyId,TrustRecordState.EFFICIENT,myLoadName,time,time);
		}else if(myLoadId!=null&&myLoadName!=null){//考虑兼容历史数据
			hql = "from TrustRecord d where d.companyId=? and d.state=? and ((d.trustor=? and d.trustorId is null) or d.trustorId=?)  and d.beginTime<=? and d.endTime>=? and d.style=2";
			main=this.delegateMainDao.findUnique(hql, companyId,TrustRecordState.EFFICIENT,myLoadName,myLoadId,time,time);
		}else if(myLoadName==null&&myLoadId!=null){//生成任务时，myLoadName直接传null过来，便于处理
			hql = "from TrustRecord d where d.companyId=? and d.state=? and d.trustorId=? and d.beginTime<=? and d.endTime>=? and d.style=2";
			main=this.delegateMainDao.findUnique(hql, companyId,TrustRecordState.EFFICIENT,myLoadId,time,time);
		}
		if(main!=null){
			if(main.getTrusteeId()==null){
				return main.getTrustee();
			}else{
				return main.getTrusteeId()+"";
			}
	...
	private String startDelegateMain(TrustRecord delegateMain){
		...
		if(delegateMain.getStyle()==3){
			...
			if(delegateMain.getBeginTime().compareTo(cal.getTime())<=0&&delegateMain.getEndTime().compareTo(cal.getTime())>=0){
				User trustee=null;
				User trustor=null;
				if(delegateMain.getTrusteeId()==null){
					trustee=ApiFactory.getAcsService().getUserByLoginName(delegateMain.getTrustee());
				}else{
					trustee=ApiFactory.getAcsService().getUserById(delegateMain.getTrusteeId());
				}
				if(delegateMain.getTrustorId()==null){
					trustor=ApiFactory.getAcsService().getUserByLoginName(delegateMain.getTrustor());
				}else{
					trustor=ApiFactory.getAcsService().getUserById(delegateMain.getTrustorId());
				}
	...
	private String endDelegateMain(TrustRecord delegateMain) {
		if(delegateMain.getStyle()==3&&delegateMain.getState()==TrustRecordState.EFFICIENT){
			User trustee=null;
			if(delegateMain.getTrusteeId()==null){
				trustee=ApiFactory.getAcsService().getUserByLoginName(delegateMain.getTrustee());//受托人
			}else{
				trustee=ApiFactory.getAcsService().getUserById(delegateMain.getTrusteeId());//受托人
			}
			User trustor=null;
			if(delegateMain.getTrustorId()==null){
				trustor=ApiFactory.getAcsService().getUserByLoginName(delegateMain.getTrustor());//受托人
			}else{
				trustor=ApiFactory.getAcsService().getUserById(delegateMain.getTrustorId());//受托人
			}
			if(trustor!=null&&trustee!=null){
				//权限委托的中止
	...
	public List<String> getConsignerByTrustee(String trustee,Long companyId,String processId,String activityName){
		...
		List<String> consigners=this.delegateMainDao.find("select d.trustor from TrustRecord d where d.companyId=?  and d.state=? and d.trustee=? and d.processId=? and d.activityName=? and d.beginTime<=? and d.endTime>=? and d.style=1 and d.trustorId is null",
	 ...
	 * @return
	 */
	public List<Long> getConsignerIdByTrusteeId(Long trusteeId,Long companyId,String processId,String activityName){
		Calendar cal = Calendar.getInstance();
		cal.setTime(new Date());
		cal.set(Calendar.HOUR_OF_DAY, 0);
		cal.set(Calendar.MILLISECOND, 0);
		...
		List<Long> trustorIds= getAllConsignerIdByTrusteeId(trusteeId,companyId,cal.getTime());
		if(trustorIds.size()>0)return trustorIds;
		
		List<Long> consignerIds=this.delegateMainDao.find("select d.trustorId from TrustRecord d where d.companyId=?  and d.state=? and (d.trusteeId is not null and d.trusteeId=?) and d.processId=? and d.activityName=? and d.beginTime<=? and d.endTime>=? and d.style=1 and d.trustorId is not null",
				companyId,TrustRecordState.EFFICIENT,trusteeId,processId,activityName,cal.getTime(),cal.getTime());
		return consignerIds;
	}
	/**
	 * 根据受托人获得委托人集合
	...
	public List<String> getAllConsignerByTrustee(String trustee,Long companyId,Date time){
		List<String> names=this.delegateMainDao.find("select d.trustor from TrustRecord d where d.companyId=? and d.state=? and d.trustee=? and d.beginTime<=? and d.endTime>=? and d.style=2 and d.trustorId is null", companyId,TrustRecordState.EFFICIENT,trustee,time,time);
		return names;
	}
	/**
	 * 根据受托人获得委托人集合
	 * @return
	 */
	public List<Long> getAllConsignerIdByTrusteeId(Long trusteeId,Long companyId,Date time){
		List<Long> trustorIds=this.delegateMainDao.find("select d.trustorId from TrustRecord d where d.companyId=? and d.state=? and (d.trusteeId is not null and d.trusteeId=?) and d.beginTime<=? and d.endTime>=? and d.style=2 and d.trustorId is not null", companyId,TrustRecordState.EFFICIENT,trusteeId,time,time);
		return trustorIds;
	}
