...
public class WorkflowAttachmentDao extends HibernateDao<WorkflowAttachment, Long>{
	...
	public WorkflowAttachment getAttachment(float fileSize,String fileName,String fileType){
		String hql = "from WorkflowAttachment d where d.fileSize=? and d.fileName=? and d.fileType=?";
		return this.findUnique(hql, fileSize,fileName,fileType);
	}
	
	public List<WorkflowAttachment> getAttachments(Long companyId) {
		if(companyId==null){
			return this.findNoCompanyCondition("from WorkflowAttachment u ");
		}else{
			return this.find("from WorkflowAttachment u where u.companyId=?",companyId);
		}
	}
