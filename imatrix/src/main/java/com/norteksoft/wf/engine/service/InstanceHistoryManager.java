...
public class InstanceHistoryManager {
	...
	private void processTasks(List<WorkflowTask> tasks, 
		...
		boolean hasBranch = acsApiManager.hasBranch(companyId);
		String userName = "";
		List<String> candiLoginNames=new ArrayList<String>();
		List<String> loginNames=new ArrayList<String>();
		List<Long> candiIds=new ArrayList<Long>();
		...
		for(WorkflowTask task : tasks){
			...
			if(TaskState.DRAW_WAIT.getIndex().equals(task.getActive())){//未领取的，
				if(task.getTransactorId()==null){
					...
					candiIds.add(task.getTransactorId());
				}
			}else{//已领取的任务
				if(task.getTransactorId()==null){
					loginNames.add(task.getTransactor());
				}else{
					userIds.add(task.getTransactorId());
				}
			}
				...
				for(WorkflowTask task : tasks){
					if(TaskState.DRAW_WAIT.getIndex().equals(task.getActive())){//未领取的，
						if(u.getLoginName().equals(task.getTransactor())){
							userName = u.getName();
							if(hasBranch){
								userName = userName+"("+u.getSubCompanyName()+")";
							}
							if(task.isSpecialTask()){
		...
		if(candiIds.size()>0){
			results=ApiFactory.getAcsService().getUsersByIds(companyId,candiIds);
			Collections.sort(results, new Comparator<User>() {
				public int compare(User user1, User user2) {
					if(user1.getWeight()<user2.getWeight()){
						return 1;
			...
			for(User u:results){
				for(WorkflowTask task : tasks){
					if(TaskState.DRAW_WAIT.getIndex().equals(task.getActive())){//未领取的，
						if(u.getId().equals(task.getTransactorId())){
							userName = u.getName();
							if(hasBranch){
								userName = userName+"("+u.getSubCompanyName()+")";
							}
		...
		if(loginNames.size()>0){
			results=ApiFactory.getAcsService().getUsersByLoginNames(companyId,loginNames);
			Collections.sort(results, new Comparator<User>() {
				public int compare(User user1, User user2) {
					if(user1.getWeight()<user2.getWeight()){
						return 1;
		...
		if(userIds.size()>0){
			results=ApiFactory.getAcsService().getUsersByIds(companyId,userIds);
			Collections.sort(results, new Comparator<User>() {
				public int compare(User user1, User user2) {
					if(user1.getWeight()<user2.getWeight()){
						return 1;
			...
			for(User u:results){
				for(WorkflowTask task : tempTasks){
					if(!TaskState.DRAW_WAIT.getIndex().equals(task.getActive())){//已领取的任务
						if(u!=null&&u.getId().equals(task.getTransactorId())){
							userName = u.getName();
							if(hasBranch){
								userName = userName+"("+u.getSubCompanyName()+")";
							}
