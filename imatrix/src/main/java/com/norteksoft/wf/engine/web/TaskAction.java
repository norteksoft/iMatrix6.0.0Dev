...
public class TaskAction extends CrudActionSupport<WorkflowTask>{
	...
	private List<Long> transactorIds = new ArrayList<Long>();
	private List<String> transactorLoginNames = new ArrayList<String>();
	...
	private Map<Long,com.norteksoft.product.api.entity.User> canChoiceTransactor = new HashMap<Long,com.norteksoft.product.api.entity.User>();
	...
	private boolean hasBranch;//是否存在分支
	...
	public String goback() throws Exception{
		...
		try {
			...
			case SINGLE_TRANSACTOR_CHOICE:
				...
				User temp;
				for(Long tran : result.getCanChoiceTransactorId()){
					temp = userManager.getUserById(tran);
					if(temp!=null){
						if(hasBranch){
							canChoiceTransacators=canChoiceTransacators+tran+","+temp.getName()+"/"+temp.getSubCompanyName()+","+temp.getLoginName()+";";
						}else{
							canChoiceTransacators=canChoiceTransacators+tran+","+temp.getName()+","+temp.getLoginName()+";";
	...
	public String taskJumpChoiceTransactor()throws Exception {
		...
		for(String info:infos){
			...
			com.norteksoft.product.api.entity.User user = new com.norteksoft.product.api.entity.User();
			user.setName(strs[1]);
			user.setLoginName(strs[2]);
			canChoiceTransactor.put(Long.parseLong(strs[0]), user);
		}
		...
		log.debug(new StringBuilder("*** Received parameter:[")
			...
			try{
				...
				case SINGLE_TRANSACTOR_CHOICE:
					...
					for(Long tran : result.getCanChoiceTransactorId()){
						temp = ApiFactory.getAcsService().getUserById(tran);
						if(temp!=null){
							if(hasBranch){
								temp.setName(temp.getName()+"/"+temp.getSubCompanyName());
							}
							canChoiceTransactor.put(tran, temp);
						}
					}
	...
	public String completeChoiceTransactor() throws Exception {
		...
		userids.addAll(Arrays.asList(transactors.toString().replace("[", "").replace("]", "").split(",")));//将id改为文本类型
		taskService.completeChoiceTransactor(taskId,userids);	
		ApiFactory.getBussinessLogService().log("工作流管理", 
				"指定办理人", 
				ContextUtils.getSystemId("wf"));
	...
	public String distribute() throws Exception{
		List<String> userids = new ArrayList<String>();
		userids.addAll(Arrays.asList(transactors.toString().replace("[", "").replace("]", "").split(",")));//将id改为文本类型
		CompleteTaskTipType completeTaskTipType = taskService.completeDistributeTask(taskId, userids);
	...
	public String changeTransactorSave() throws Exception{
		taskService.changeTransactor(taskId, Long.parseLong(transactor));
		task = taskService.getWorkflowTask(taskId);
		workflowId = task.getProcessInstanceId();
		workflowInstance = workflowInstanceManager.getWorkflowInstance(workflowId);
		//更改办理人业务补偿
	...
	public String addTask() throws Exception {
		List<String> userids = new ArrayList<String>();
		userids.addAll(Arrays.asList(transactors.toString().replace("[", "").replace("]", "").split(",")));//将id改为文本类型
		taskService.generateTask(taskId,userids,TaskSource.ADD_SIGN,true);
		ApiFactory.getBussinessLogService().log("工作流平台", 
				"加签", 
				ContextUtils.getSystemId("wf"));
		return null;
	...
	public String completeCopyTache() throws Exception {
		List<String> userids = new ArrayList<String>();
		userids.addAll(Arrays.asList(transactors.toString().replace("[", "").replace("]", "").split(",")));//将id改为文本类型
		taskService.createCopyTaches(taskId,userids,"","/engine/task!input.htm");	
		ApiFactory.getBussinessLogService().log("工作流平台", 
				"完成抄送任务", 
				ContextUtils.getSystemId("wf"));
	...
	public String delTransactorSave() throws Exception{
		taskService.delTransactor(workflowId,transactorLoginNames, transactorIds);
		ApiFactory.getBussinessLogService().log("流程监控", 
				"减少办理人", 
				ContextUtils.getSystemId("wf"));
	...
	public String delTransactor() throws Exception {
		...
		if(!isForkTask){//表示是分支汇聚任务时不让减少办理人
			List<WorkflowTask> currentTasks=taskService.getActivityTaskByInstance(workflowId);
			List<WorkflowTask> currentPrincipalTasks=taskService.getActivityPrincipalTask(workflowId);
			if(currentTasks!=null){
				if(currentTasks.size()==1){
					WorkflowTask task=currentTasks.get(0);
					hasBranch = ContextUtils.hasBranch();
					if(hasBranch){
						transactor=task.getTransactorName()+"/"+task.getTransactorSubCompanyName();
					}else{
						transactor=task.getTransactorName();
					}
				}
				transactorNum=currentTasks.size();
			}
			if(currentPrincipalTasks!=null){
				if(transactorNum==0&&currentPrincipalTasks.size()==1){
					WorkflowTask task=currentPrincipalTasks.get(0);
					hasBranch = ContextUtils.hasBranch();
					if(hasBranch){
						transactor=task.getTrustorName()+"/"+task.getTrustorSubCompanyName();
					}else{
						transactor=task.getTrustorName();
					}
				}
				transactorNum=transactorNum+currentPrincipalTasks.size();
			}
		}
		//判断当前环节是否是子流程
	...
	public String delTransactorTree() throws Exception{
		...
		ZTreeNode node = null;
		String name = ContextUtils.getCompanyName();
		...
		node = new ZTreeNode("company_company-company~company","0",name, "true", "false", "", "", "root", "");
		treeNodes.add(node);
			...
			for(WorkflowTask t : taskList){
				if(hasBranch){
					if(StringUtils.isEmpty(t.getTrustorName())){
						node = new ZTreeNode("user_"+ t.getTransactorName()+"-"+t.getTransactor()+"~"+(t.getTransactorId()==null?"":t.getTransactorId()),"company_company-company~company",t.getTransactorName()+"/"+t.getTransactorSubCompanyName(), "true", "false", "", "", "user", "");
					}else{
						node = new ZTreeNode("user_"+ t.getTransactorName()+"-"+t.getTransactor()+"~"+(t.getTransactorId()==null?"":t.getTransactorId()),"company_company-company~company",t.getTransactorName()+"/"+t.getTransactorSubCompanyName()+"（受托于"+t.getTrustorName()+"/"+t.getTrustorSubCompanyName()+")", "true", "false", "", "", "user", "");
					}
				}else{
					if(StringUtils.isEmpty(t.getTrustorName())){
						node = new ZTreeNode("user_"+ t.getTransactorName()+"-"+t.getTransactor()+"~"+(t.getTransactorId()==null?"":t.getTransactorId()),"company_company-company~company",t.getTransactorName(), "true", "false", "", "", "user", "");
					}else{
						node = new ZTreeNode("user_"+ t.getTransactorName()+"-"+t.getTransactor()+"~"+(t.getTransactorId()==null?"":t.getTransactorId()),"company_company-company~company",t.getTransactorName()+"（受托于"+t.getTrustorName()+")", "true", "false", "", "", "user", "");
					}
				}
				treeNodes.add(node);
			}
		}
		renderText(JsonParser.object2Json(treeNodes).toString());
		return null;
	}
	...
	public Map<Long, com.norteksoft.product.api.entity.User> getCanChoiceTransactor() {
	...
	public void setTransactorLoginNames(List<String> transactorLoginNames) {
		this.transactorLoginNames = transactorLoginNames;
	}

	public void setTransactorIds(List<Long> transactorIds) {
