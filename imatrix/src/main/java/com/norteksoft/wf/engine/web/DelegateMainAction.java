...
public class DelegateMainAction extends CrudActionSupport<TrustRecord>{
	...
	public String view() throws Exception {
		delegateMain.setTrustorName(userManager.getUserById(ContextUtils.getUserId()).getName());
		ApiFactory.getBussinessLogService().log("委托", 
				"查看委托", 
				ContextUtils.getSystemId("wf"));
		return "view";
	...
	public String viewReceive() throws Exception {
		delegateMain.setTrustorName(userManager.getUserById(delegateMain.getTrustorId()).getName());
		ApiFactory.getBussinessLogService().log("委托", 
				"查看受托", 
				ContextUtils.getSystemId("wf"));
		return "viewReceive";
	...
	public String save() throws Exception {
		if(validateDelegateDate(delegateMain)==0){
			if(!ContextUtils.getUserId().equals(delegateMain.getTrusteeId())){
				...
				delegateMain.setTrustorId(ContextUtils.getUserId());
	...
	public String getLink() throws Exception{
		workflowDefinitions=workflowDefinitionManager.getAllActiveDefinition();
		WorkflowDefinition def = workflowDefinitionManager.getWorkflowDefinitionByProcessId(processDefinitionId);
		if(def!=null){
			List<ZTreeNode> treeNodes = new ArrayList<ZTreeNode>();
			ZTreeNode root = new ZTreeNode(def.getId()+"","0", def.getName()+"("+def.getVersion()+")", "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			taskNames=workflowDefinitionManager.getTaskNames(processDefinitionId);
			for (String taskName : taskNames) {
				root = new ZTreeNode(taskName,def.getId()+"", taskName, "false", "false", "", "", "folder", "");
				treeNodes.add(root);
			}
	...
	public String myDelegate()throws Exception{
		if(tasks.getPageSize()>1){
			if(isEnd == null || isEnd.length() == 0){
				tasks = taskService.getDelegateTasksByActive(ContextUtils.getCompanyId(), tasks, ContextUtils.getLoginName(),ContextUtils.getUserId(),false);
			}else{
				tasks = taskService.getDelegateTasksByActive(
						ContextUtils.getCompanyId(), tasks, ContextUtils.getLoginName(),ContextUtils.getUserId(), Boolean.valueOf(isEnd));
			}
			ApiFactory.getBussinessLogService().log("委托", 
					"任务委托监控列表", 
					ContextUtils.getSystemId("wf"));
	...
	public String myHistoryDelegate()throws Exception{
		if(historyTasks.getPageSize()>1){
			historyTasks = taskService.getHistoryDelegateTasksByActive(
					ContextUtils.getCompanyId(), historyTasks, ContextUtils.getLoginName(),ContextUtils.getUserId());
		...
		if(inHistory){
			if(historyTasks.getPageSize()>1){
				historyTasks = historyWorkflowTaskManager.getTaskAsTrustee(
						ContextUtils.getCompanyId(), historyTasks, ContextUtils.getLoginName(),ContextUtils.getUserId(), isDone);
				ApiFactory.getBussinessLogService().log("委托", 
						"任务受托监控列表", 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(historyTasks));
			...
			if(tasks.getPageSize()>1){
				tasks = taskService.getTaskAsTrustee(
						ContextUtils.getCompanyId(), tasks, ContextUtils.getLoginName(),ContextUtils.getUserId(), isDone);
				ApiFactory.getBussinessLogService().log("委托", 
						"任务受托监控列表", 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(tasks));
	...
	public void prepareMyDelegateInput() throws Exception{
		...
		if(delegate.getTransactorId()==null){
			delegate.setExpands(userManager.getUserByLoginName(delegate.getTransactor()).getName());
		}else{
			delegate.setExpands(userManager.getUserById(delegate.getTransactorId()).getName());
		}
	}
	...
	public String getRolesByUser() throws Exception{
		...
		for(Role role : roles){
			...
			if(businessSystem!=null){
				...
				roleName.append("/");
				roleName.append(role.getSubCompanyName());
				roleName.append(")");
			}
			htm.append("<li>");
			if(id==null){
