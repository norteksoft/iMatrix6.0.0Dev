...
public class WorkflowTypeDao extends HibernateDao<WorkflowType, Long>{
	...
	public WorkflowType getWorkflowType(String typeCode) {
		return getWorkflowType(typeCode,ContextUtils.getCompanyId());
	}
	public WorkflowType getWorkflowType(String typeCode,Long companyId) {
		List<WorkflowType>  types = find("from WorkflowType wt where wt.code=? and wt.companyId = ? ",typeCode,companyId);
