...
public class OfficeDao extends HibernateDao<Document, Long>{
	...
	public List<Document> getDocuments(Long companyId) {
		if(companyId==null){
			return this.findNoCompanyCondition("from Document d ");
		}else{
			return this.find("from Document d where d.companyId=?",companyId);
		}
