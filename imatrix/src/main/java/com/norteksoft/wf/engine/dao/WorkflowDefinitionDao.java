...
public class WorkflowDefinitionDao extends HibernateDao<WorkflowDefinition, Long>{
	...
	public void getWfDefinitions(Page<WorkflowDefinition> page,Long companyId,String enableOrDisable,String adminCode,boolean  isSuperWf,Long adminId){
		if(isSuperWf){//是否是流程定义管理员
			if(enableOrDisable.equals("UNABLE")){
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and wfd.enable=? and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by name,version desc",companyId,DataState.DISABLE,adminCode,adminId);
			}else{
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and (wfd.enable=? or wfd.enable=? ) and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by enable asc ,name,version desc",companyId,DataState.ENABLE,DataState.DRAFT,adminCode,adminId);
			}
		}else{
			if(enableOrDisable.equals("UNABLE")){
	...
	public void getWfDefinitions(Page<WorkflowDefinition> page,Long companyId,Long type,String enableOrDisable,String adminCode,boolean  isSuperWf,Long adminId){
		if(isSuperWf){//是否是流程定义管理员
			if(enableOrDisable.equals("UNABLE")){
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and wfd.typeId=?  and wfd.enable=? and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by name,version desc" ,companyId,type,DataState.DISABLE,adminCode,adminId);
			}else{
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and wfd.typeId=?  and (wfd.enable=? or wfd.enable=? ) and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by enable asc ,name,version desc" ,companyId,type,DataState.ENABLE,DataState.DRAFT,adminCode,adminId);
			}
		}else{
			if(enableOrDisable.equals("UNABLE")){
	...
	public void getWfDefinitionsBySystemId(Page<WorkflowDefinition> page,Long companyId,Long systemId,String enableOrDisable,String adminCode,boolean isSuperWf,Long adminId){
		if(isSuperWf){//是否是流程定义管理员
			if(enableOrDisable.equals("UNABLE")){
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and wfd.systemId=? and wfd.enable=? and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by name,version desc" ,companyId,systemId,DataState.DISABLE,adminCode,adminId);
			}else{
				searchPageByHql(page, "from WorkflowDefinition wfd where wfd.companyId = ? and wfd.systemId=? and (wfd.enable=? or wfd.enable=? ) and ((wfd.adminLoginName=? and wfd.adminId is null) or (wfd.adminId is not null and wfd.adminId = ?)) order by enable asc , name,version desc" ,companyId,systemId,DataState.ENABLE,DataState.DRAFT,adminCode,adminId);
			}
		}else{
			if(enableOrDisable.equals("UNABLE")){
	...
	public WorkflowDefinition getWorkflowDefinitionByCodeAndVersion(String definitionCode,
		...
		if(isSuperWf){//是流程定义管理员
			return this.findUnique("from WorkflowDefinition ft where ft.code=? and ft.version=? and ft.companyId = ?  and ((ft.adminLoginName=? and ft.adminId is null) or (ft.adminId is not null and ft.adminId = ?))", definitionCode,definitionVersion,companyId,ContextUtils.getLoginName(),ContextUtils.getUserId());
		}else{
