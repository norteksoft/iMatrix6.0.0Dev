...
public class DefinitionXmlParse {
    ...
    private static Map<String, String> getMyProcessBaseInfo(Document doc){
        ...
        if(propElement!=null)properties.put("branch-name", propElement.getText());
        propElement = Dom4jUtils.getSubElementByName(basePropElement, "branch-code");
        if(propElement!=null)properties.put("branch-code", propElement.getText());
        propElement = Dom4jUtils.getSubElementByName(basePropElement, "creator-name");
        if(propElement!=null)properties.put("creator-name", propElement.getText());
        propElement = Dom4jUtils.getSubElementByName(basePropElement, "currentor-loginname");
        if(propElement!=null)properties.put("currentor-loginname", propElement.getText());
        propElement = Dom4jUtils.getSubElementByName(basePropElement, "currentor-name");
        if(propElement!=null)properties.put("currentor-name", propElement.getText());
    	return properties;
    }
    ...
    public static void transitionInformSwing(String processId,String transitionName,String receivers,WorkflowInstance workflow){
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				User user = UserUtil.getUserByInfo(loginName);
    				if(user!=null){
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),"待办任务流向通知",getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName);
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),"待办任务流向通知",getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName);
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
    	...
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				User user = UserUtil.getUserByInfo(loginName);
    				if(user!=null){
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),"流程结束时通知",getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId);
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),"流程结束时通知",getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId);
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//	}
	...
	public static final String CURRENTOR_NAME = "currentor-name";
	public static final String CURRENTOR_LOGINNAME = "currentor-loginname";
