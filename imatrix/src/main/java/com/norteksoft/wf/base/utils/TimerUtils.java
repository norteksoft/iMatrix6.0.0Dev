...
public class TimerUtils {
	...
	public void RtxInform(WorkflowTask task) {
		...
		if(StringUtils.isNotEmpty(task.getReminderNoticeUser())){
			for(String userId : task.getReminderNoticeUser().split(",")){
				com.norteksoft.product.api.entity.User user = ApiFactory.getAcsService().getUserById(Long.parseLong(userId));
				if(user!=null){
					rtx.RtxMsgSender.sendNotify(user.getLoginName(), "催办超期提醒", "1", msg , "",task.getCompanyId());
				}
			}
		}	
	}
	...
	public void swingInform(WorkflowTask task) throws Exception {
		...
		if(StringUtils.isNotEmpty(task.getReminderNoticeUser())){
			for(String userId : task.getReminderNoticeUser().split(",")){
				com.norteksoft.product.api.entity.User user = ApiFactory.getAcsService().getUserById(Long.parseLong(userId));
				if(user!=null){
					ApiFactory.getPortalService().addMessage("task", "系统管理员", ContextUtils.getUserId(), user.getId(),"催办超期提醒", msg, "/task/message-task.htm?id="+task.getId());
				}
			}
		}	
	...
	public void swingReminder(WorkflowTask task) throws Exception{
		if(StringUtils.isNotEmpty(task.getTransactor())){
			...
			if(task.getTransactorId()==null){
				...
				ApiFactory.getPortalService().addMessage("task", "系统管理员", ContextUtils.getUserId(), task.getTransactorId(),"待办任务催办提醒", msg, "/task/message-task.htm?id="+task.getId());
	...
	public boolean neetReminder(WorkflowTask task){
		return (task.getLastReminderTime()== null && ((System.currentTimeMillis()-task.getCreatedTime().getTime())>task.getDuedate()*MILLI_SECOND) && (!isHolidayDate(task.getSubCompanyId()))) ||
		(task.getLastReminderTime()!= null && ((System.currentTimeMillis()-task.getLastReminderTime().getTime())>task.getRepeat()*MILLI_SECOND) && (!isHolidayDate(task.getSubCompanyId())));
	}
	...
	public boolean isHolidayDate(Long subCompanyId){
		...
		List<Integer> holidaySetting = holidayManager.getMonthSetting(tomorrow,subCompanyId);
