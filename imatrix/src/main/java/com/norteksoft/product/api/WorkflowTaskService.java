...
public interface WorkflowTaskService {
	 ...
	 * @param taskId
	 * @param allOriginalUsers 下一环节原办理人
	 * @param transcators 下一环节办理人，当存在多分支机构，且有同名用户时，该办理人集合请传用户id，不要传用户登录名
	 * @return CompleteTaskTipType {@link com.norteksoft.wf.base.enumeration.CompleteTaskTipType}
	 */
     ...
     * 加签
     * @param taskId 任务id
     * @param users 需要加签的人,当存在多分支机构，且有同名用户时，该办理人集合请传用户id，不要传用户登录名
     */
    ...
    */
    ...
    /**
     * 减签
     * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>removeSigners(Long taskId,Collection<Long> userIds);</code>
     * @param taskId 任务id
     * @param users 需要减签的人登录名
     */
    public void removeSigner(Long taskId,Collection<String> users);
    /**
     * 减签
     * @param taskId 任务id
     * @param userIds 需要减签的人id
     */
    ...
    /**
     * 减签
     * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>removeSigner(Long taskId,Long... users);</code>
     * @param taskId 任务id
     * @param users 需要减签的人
     */
     ...
     * @param users 需要减签的人
     */
    public void removeSigner(Long taskId,Long... users);
	 ...
	 * 交办 或 指派 <br/>
     * 将任务交办或指派给指定的人员。
     * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>assign(Long taskId, Long assignee);</code>
	 ...
	 */
	public void assign(Long taskId, Long assignee);
	...
	/**
	 * 查询办理人的当前任务.
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>WorkflowTask getActiveTaskByTransactorId(FormFlowable entity,Long transactorId);</code>
	 * @param entity 走流程的表单
	 * @param loginName 办理人的登录名
	 * @return 如果流程已经启动 ，返回当前任务；否则，返回null
	 */
	...
	/**
	 * 查询流程定义中某个办理人的任务
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>getWorkflowTasksByDefinitonName(String definitionName,Long userId);</code>
	 ...
	 * @return 任务列表
	 */
	public List<WorkflowTask> getWorkflowTasksByDefinitonName(String definitionName,Long userId);
	/**
	 * 生成抄送任务
	 * @param taskId 当前任务的id
	 ...
	 * 参数title为null或空字符串时，生成的该抄送任务的标题为“(抄送)原任务标题”;参数url为null或空字符串时，生成的抄送任务的办理页面为该url
	 * @param taskId 当前任务的id
	 * @param transactors 将该任务抄送给谁,当存在分支机构，且分支之间存在同名用户时，请传用户id的集合
	 */
	...
	/**
	 * 根据用户查询未完成任务总数
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>getActiveTaskCountByTransactorId(Long userId);</code>
	 * @param loginName 用户登录名
	 * @return 未完成任务总数
	 */
	...
	/**
	 * 查询每个人的超期任务数,包括已完成的任务
	 * @return 返回值为HashMap,key为用户id,value为超期任务总数
	 */
	...
	/**
	 * 分页查询用户所有未完成任务
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>getUnDoneTasksByUser( Long userId, Page<com.norteksoft.task.entity.WorkflowTask> page);</code>
	 * @param loginName 用户登录名
	 * @param page
	 */
	@Deprecated
	 ...
	 */
	@Deprecated
	public void getUnDoneTasksByUser( Long userId, Page<com.norteksoft.task.entity.WorkflowTask> page);
	...
	/**
	 * 查询用户所有未完成任务(不是分页)
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>getActiveTasksByUserId(Long userId);</code>
	 * @param loginName 用户登录名
	 * @return 用户所有未完成任务集合
	 */
	...
	*/
	...
	/**
	 * 查询当前待办理的任务的办理人,不包括受托任务，即trustor为null
	 * @param entity  实体
	 * @return
	 */
	...
	/**
	 * 判断用户是否有任务
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用<code>userHasTask(Long transactorId,String workflowId);</code>
	 * @param trasactor 办理人登录名
	 * @param workflowId 实例id
	 * @return
	 */
	...
	/**
	 * 获得当前实例中当前环节的办理人登录名集合
	 * @param entity
	 * @param taskName
	 * @return
	 */
	public Set<String> getTransactorsByTaskName(FormFlowable entity,String taskName);
	/**
	 * 获得当前实例中当前环节的办理人登录名集合
	 * @param entity
	 * @param taskName
	 * @return
	 */
	public Set<String> getTransactorsByTaskName(String workflowId,String taskName);
	 ...
	 * @return
	 */
	public Set<Long> getTransactorIdsByTaskName(FormFlowable entity,String taskName);
	/**
	 * 获得当前实例中当前环节的办理人id集合
	 * @param entity
	 * @param taskName
	 * @return
	 */
	public Set<Long> getTransactorIdsByTaskName(String workflowId,String taskName);
}
