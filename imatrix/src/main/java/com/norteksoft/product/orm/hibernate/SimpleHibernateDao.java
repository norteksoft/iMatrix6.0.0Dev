...
public class SimpleHibernateDao<T, PK extends Serializable> {
	...
	public String getAlias(String hql){
		...
		if(!hql.contains(group_by) && hql.contains("GROUP BY")) order_by = "GROUP BY";
		
		String fromHql = StringUtils.substringAfter(hql, from);
		fromHql = StringUtils.substringBefore(fromHql, group_by);
	...
	protected String addCompanyCondition(final String hql){
		...
		// 包含 group by
		String groupBy = "";
		if(hql.contains(group_by)){
			groupBy = " group by " + StringUtils.substringAfter(hql, group_by);
		}
		// 包含 where 子句
		if(hql.contains(where)){
			newHql.append(StringUtils.substringBefore(hql, where));
			String whereCondition = StringUtils.substringBefore(hql, group_by);
			whereCondition = StringUtils.substringBefore(whereCondition, order_by);
			...
			if(StringUtils.isNotBlank(groupBy)){
				newHql.append(groupBy);
			}else if(StringUtils.isNotBlank(orderBy)){
				newHql.append(orderBy);
			}
		}else{
			if(StringUtils.isNotBlank(groupBy)){// 不包含 where , 有 group by
				newHql.append(StringUtils.substringBefore(hql, group_by));
				newHql.append(" where ");
				newHql.append(getAlias(hql)).append(".").append(COMPANY_ID).append("=");
				newHql.append(getCompanyId());
				newHql.append(groupBy);
			}else if(StringUtils.isNotBlank(orderBy)){// 不包含 where , 没有group by , 有 order by
				newHql.append(StringUtils.substringBefore(hql, order_by));
				newHql.append(" where ");
				newHql.append(getAlias(hql)).append(".").append(COMPANY_ID).append("=");
				newHql.append(getCompanyId());
				newHql.append(orderBy);
			}else{// 不包含 where , 没有group by , 没有 order by
