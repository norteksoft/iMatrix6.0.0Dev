...
public class CornBuilder {
	...
	public static Trigger builderByCornInfo(Timer info){
		...
		case appointSet:  // 0 15 10 * * ? 2005
			...
			break;
		case intervalTime:  //调用事件间隔api
			if(StringUtils.isNotEmpty(info.getEverySecond())){
			    tg = CalendarIntervalScheduleBuilder
				.calendarIntervalSchedule()
				.withIntervalInMinutes(Integer.valueOf(info.getEverySecond())).build()
				.getTriggerBuilder().build();
			}
			if(StringUtils.isNotEmpty(info.getEveryHour())){
				tg = CalendarIntervalScheduleBuilder
					.calendarIntervalSchedule()
					.withIntervalInHours(Integer.valueOf(info.getEveryHour())).build()
					.getTriggerBuilder().build();
			}
			break;
		}
