...
public abstract class WorkflowManagerSupport<T extends FormFlowable> {
	 ...
	 * @return
	 */
	public WorkflowTask getActiveTaskByTransactorId(T t, Long userId) {
		return ApiFactory.getTaskService().getActiveTaskByTransactorId(t,
				userId);
	}
	...
	/**
	 * 减签。
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请调用removeSigners(Long taskId, List<Long> lists);
	 * @param taskId
	 * @param lists
	 */
	@Deprecated
	 ...
	 * @param lists
	 */
	public void removeSigners(Long taskId, List<Long> lists) {
		ApiFactory.getTaskService().removeSigners(taskId,lists);
	}
	...
	/**
	 * 抄送任务。
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题，请传用户id的集合
	 * @param taskId
	 * @param transactors
	 * @param title
	 * @param url
