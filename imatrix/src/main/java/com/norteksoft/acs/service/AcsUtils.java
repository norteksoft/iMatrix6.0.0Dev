...
public class AcsUtils {
	...
	public Set<Role> getRolesByUser(Long systemId, Long companyId, Long userId){
		...
		rolesByBranchesHql.append("select r from RoleDepartment du join du.role r join du.department d where r.deleted=? and r.businessSystem.id=? and du.deleted=? and d.deleted=? and d.branch=? and d.company.id=? and d.id is (select u.subCompanyId from User u where u.id=? and u.deleted=? and u.subCompanyId is not null)");
		List<Role> branchesRoles = roleDao.find(rolesByBranchesHql.toString(), false,systemId, false, false,true,companyId,userId, false);
	 ...
	 * @param roleName
	 * @return
	 * 把该方法标注为过时的原因：含有分支机构的集团公司角色名称在同一系统同一分支机构下唯一
	 */
	@Deprecated
	@Transactional(readOnly = true)
