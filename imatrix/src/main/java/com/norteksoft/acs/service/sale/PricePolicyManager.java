...
public class PricePolicyManager {
	...
	/**
	 * 根据产品ID及价格策略查询价格策略是否存在
	 * @param page
	 * @param productId
	 */
	public PricePolicy getPricePolicyByInfo(Long productId,String priceName,BigDecimal amount){
		String hql = "select pp from PricePolicy pp where (pp.product!=null and pp.product.id = ?) and pp.priceName= ? and pp.amount = ?";
		List<PricePolicy>  list = pricePolicyDao.find( hql, productId,priceName,amount);
