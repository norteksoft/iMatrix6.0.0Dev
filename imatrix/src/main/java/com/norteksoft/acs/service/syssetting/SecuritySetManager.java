...
public class SecuritySetManager {
	...
	public Set<Role> getRolesByUser(Long userId){
		...
		rolesByBranchesHql.append("select r from RoleDepartment du join du.role r join du.department d where r.deleted=? and r.businessSystem.id=? and du.deleted=? and d.deleted=? and d.branch=? and d.id is (select u.subCompanyId from User u where u.id=? and u.deleted=? and u.subCompanyId is not null)");
		List<Role> branchesRoles = roleDao.find(rolesByBranchesHql.toString(), false,ContextUtils.getSystemId(), false, false,true , userId, false);
	...
	public Set<Role> getRolesByUserNew(Long userId){
		...
		StringBuilder rolesByBranchesHql = new StringBuilder();
		rolesByBranchesHql.append("select r from RoleDepartment du join du.role r join du.department d where r.deleted=? and du.deleted=? and d.deleted=? and d.branch=? and d.id is (select u.subCompanyId from User u where u.id=? and u.deleted=? and u.subCompanyId is not null)");
		List<Role> branchesRoles = roleDao.find(rolesByBranchesHql.toString(), false, false, false,true , userId, false);
	...
	public List<Role> getRolesByUserAndBussinessId(Long userId,Long bussinessId){
		...
		StringBuilder rolesByBranchesHql = new StringBuilder();
		rolesByBranchesHql.append("select r from RoleDepartment du join du.role r join du.department d where r.deleted=? and r.businessSystem.id=? and du.deleted=? and d.deleted=? and d.branch=? and d.id is (select u.subCompanyId from User u where u.id=? and u.deleted=? and u.subCompanyId is not null)");
		List<Role> branchesRoles = roleDao.find(rolesByBranchesHql.toString(), false,bussinessId, false, false,true , userId, false);
