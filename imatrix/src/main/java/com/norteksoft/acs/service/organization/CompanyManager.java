...
public class CompanyManager {
	...
	public List<Company> getCompanyList() {
		return companyDao.findByCriteria(Restrictions.eq("deleted", false));
	}
	...
	public Company getCompanyByCode(String code){
		List<Company> companys = companyDao.findByCriteria(Restrictions.eq("deleted", false),Restrictions.eq("code",code));
		if(companys.size()>0)return companys.get(0);
		return null;
	}
