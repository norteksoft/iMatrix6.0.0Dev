...
//	}
     ...
     * @param message   日志信息
     */
    public void debugLog(String operatorName,String operatorLoginName, String optType, String message){
    	Log log = createLog(message, null);
    	log.setOperator(operatorName);
    	log.setOperatorLoginName(operatorLoginName);
		log.setSubCompanyName(ContextUtils.getCompanyName());
    	log.setOperationType(optType);
    	logDao.save(log);
    }
    /**
     ...
     * @param message   日志信息
     */
    public void debugLog(Long operatorId, String optType, String message){
    	Log log = createLog(message, null);
    	...
    	if(user!=null){
    		...
    		log.setSubCompanyId(user.getSubCompanyId());
    		log.setSubCompanyName(user.getSubCompanyName());
...
//	}
	...
	private Log createLog(String message, Long systemId){
		...
		if(request!=null){
			String ipAddress = request.getHeader("x-forwarded-for");//有网关时取到最终服务器ip地址
			...
			log.setOperatorId(ContextUtils.getUserId());
			log.setOperatorLoginName(ContextUtils.getLoginName());
			log.setSubCompanyId(ContextUtils.getSubCompanyId());
	    	if(ContextUtils.getSubCompanyId()==null){
	    		log.setSubCompanyName(ContextUtils.getCompanyName());
	    	}else{
	    		log.setSubCompanyName(ContextUtils.getSubCompanyName());
	    	}
			log.setOperatorType(getOperatorType());
		}else{
			log.setIpAddress("0.0.0.0");
			log.setOperator("系统日志");
			log.setOperatorLoginName("系统日志");
