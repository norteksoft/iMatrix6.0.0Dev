...
public class SystemContextUtils {
	...
	protected static Log logger = LogFactory.getLog(SystemContextUtils.class);
	 ...
	 * @param companyId
	 */
	public static void setAnonymousCompanyId(Long companyId){
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		User user = new User(anonymous, anonymous, false, false, false, false, 
				new GrantedAuthority[]{new GrantedAuthorityImpl(anonymousRole)});
		...
		if(authentication == null){
			...
			((AnonymousAuthenticationToken) authentication).setDetails(user);
		}
	}
	...
	public static void setBusinessSystem(BusinessSystem businessSystem) {
		SystemContextUtils.businessSystem = businessSystem;
	}
	...
	public static void addInfo(Object key, Object value){
		Map<Object, Object> otherInfos = getCurrentUser().getOtherInfos();
		if(otherInfos == null){
			otherInfos = new HashMap<Object, Object>();
		}
		otherInfos.put(key, value);
	}
	
	public static Object getInfo(Object key){
		return getCurrentUser().getOtherInfos().get(key);
	}
